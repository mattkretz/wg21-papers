...
LEWG directed me to remove the \code{size} member from \simd and \mask for disabled specializations
...
\end{lstlisting}
Because \code{size} is not a member of disabled \simd specializations anymore, the code above is
...
\wgRem{In addition only the \tcode{value_type}, \tcode{abi_type}, and
...
\subsection{Suggested Poll}
\wgPoll{Name lookup of members of disabled \code{basic_simd} specializations works if it works for
enabled \code{basic_simd} specializations. Likewise for \code{basic_simd_mask}.}{&&&&}
