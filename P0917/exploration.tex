...
More tweaks to \code{common_type} were applied over time so that the default \code{common_type_t<T, U>} (i.e. unless specialized) is the first valid type of (if any):
\begin{enumerate}
  \item \code{decay_t<decltype(false ? declval<decay_t<T>>() : declval<decay_t<U\MayBreak>\MayBreak>())>};
  \item \code{decay_t<decltype(false ? declval<decay_t<T>(\&)()>()()}\\\code{\hspace*{12em}: declval<de\-cay_t<U>(\&)()>()())>}.
\end{enumerate}
Consequently, an overloadable conditional operator would directly affect \std\code{common_type}.

Given the connection between \code{common_type} and \code{operator?:}, the built-in conditional operator could fall back to instantiating \std\code{common_type<T, U>}, if the existing rules do not find a common type.
The compiler would then synthesize a built-in \code{operator?:}, which automatically implements lazy evaluation.

%\subsection{Deferred Evaluation}
%The R0 revision of this paper discussed how to enable deferred evaluation for non-synthesized conditional operators.
...
%
%However, since R4, this paper proposes 
...
Consequently, a conditional operator as member of a class must be a member of the condition type.
...
However, for consistency with all other operators, we might want to allow it nevertheless.
...
// most common & trivial usage of ?: with UDTs:
...
using X = decltype(A() ? B() : C());  // X = float (GCC, Clang), double (EDG),
                                      //     ill-formed (MSVC)
...
It does not work for interconvertible UDTs:
\medskip\begin{lstlisting}
struct E;
struct F { operator E(); };
struct E { operator F(); };
using Z = decltype(A() ? F() : E());  // ill-formed
\end{lstlisting}

Should the user be able to define a conditional operator that takes precedence over the built-in operator?
In any case, \code{operator?:} overloads would require at least one user-defined type in their signature.
But can and should we require that an \code{operator?:} overload does not override a built-in \code{operator?:}?
...
All operator (and function) overloads can already be used to construct such inconsistencies.
Example:
\medskip\begin{lstlisting}
...
\end{lstlisting}
Is this something we also want for \code{operator?:}.

We also need to consider backwards compatibility.
Right now the following code is well-formed:
\medskip\begin{lstlisting}
struct Incomplete;
...
int f(Testable<Wrap<Incomplete>> c, int a, int b)
{ return c ? a : b; } // OK
\end{lstlisting}

But when an overloadable operator is involved, the use of \code{c} is ill-formed:
\medskip\begin{lstlisting}
bool g(Testable<Wrap<Incomplete>> c)
{ return c && c; } // ill-formed because of operator&& lookup in Wrap<Incomplete>
\end{lstlisting}

If we make every conditional operator go through name lookup, then we risk a breaking change.
In addition, changing every use of the conditional operator to now do name lookup and (more) overload resolution makes compile times slower.

This paper proposes to make the declaration of \code{operator?:} overloads that would replace a built-in operator ill-formed.
(For \code{operator?:} templates, this check on declaration / instantiation is not needed, since templates would only be considered after no built-in conditional operator is viable.)

This approach, while inconsistent with existing operators, preserves full backwards compatibility (also with regards to compile-time performance) and eliminates the potential for inconsistent answers to the common type.

%The current specification of the conditional operator [expr.cond] mentions the use of overload resolution in paragraph 6:
