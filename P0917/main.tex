...
This leaves \code{operator?:} as the last missing piece\footnote{Maybe \code{operator::} as well. I didn't take much time to consider what it would mean, though.}.
...
Consider \cite{N4744}, where a certain number (determined at compile time) of values of arithmetic type \type T are combined to a single object of type \type{simd<T, Abi>}.
...
More importantly, if \code x is a builtin type, the function will not be found via ADL; consequently, user code requires \code{return std::experimental::iif(x < 0, -x, x)} to be generic.
...
Allowing overloads of \code{operator?:} in \CC{} enables users to implement blend semantics with the same syntax as provided by GCC and OpenCL.
...
Calling code such as \code{auto x = cond ? a + b : g(a, b)} could then be transformed to \code{auto x = operator?:(cond, [\&]() { return a + b; }, [\&]() { return g(a, b); })}.
...
If P0927R0 is not adopted, we have two choices for the signature of the conditional operator overload (cf. Figure \ref{fig:demo}):
\begin{enumerate}
...
Consider \code{template <class F0, class F1> UDT operator?:(bool, F0, F1)}, where \code{F0} and \code{F1} return a prvalue of type \code{UDT}.
The built in operator would be viable for \code{some_bool ? udt_object_a : udt_object_b} and the overload, even without transformation of the expressions into lambda.
I.e. the user-defined operator, requires a constraint that \code{F0} and \code{F1} are callables.
...
jmp b()
    \end{lstlisting}
  \end{minipage}
  \caption{Demonstration of the two choices compared to a builtin conditional operator (\code{f3}), cf. \url{https://godbolt.org/g/tpPrVR}}
  \label{fig:demo}
\end{figure}
...
Consider a conceivable implementation of the conditional operator using choice \ref{simple} for \type{simd<T, Abi>}:
\smallskip\begin{lstlisting}[style=Vc]
