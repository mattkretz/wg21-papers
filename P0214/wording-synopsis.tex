...
\begin{itemdescr}
  ...
  Additionally, for every supported \simd[<T, Abi>] (see \ref{simd.type requirements}), where \type{Abi} is an implementation-defined ABI tag, \code N $=$ \simd[<T, Abi>::size()] must be supported.
...
\begin{itemdescr}
  ...
  \wgNote{
    \code{value} identifies the alignment restrictions on pointers used for (converting) loads and stores for the given type \type T on arrays of type \type U (see \ref{sec:simd.load}, \ref{sec:simd.store}, \ref{sec:simd.mask.load}, \ref{sec:simd.mask.store}).
  }
\end{itemdescr}
