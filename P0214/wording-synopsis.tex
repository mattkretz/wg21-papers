...
\begin{itemdescr}
  ...
  Additionally, for every supported \simd[<T, Abi>] (see \ref{simd.type requirements}), where \type{Abi} is an ABI tag that is not an instance of \code{simd_abi::fixed_size<int>}, \code N $=$ \simd[<T, Abi>::size()] shall be supported.
  ...
  \wgNote{
    An implementation can choose to forego ABI compatibility between differently compiled translation units for \simd and \mask instantiations using the same \fixedsizeN tag.
