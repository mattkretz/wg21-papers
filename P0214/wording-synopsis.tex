...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks If \type M is \bool, the function only participates in overload resolution if \type U is \bool.
  \pnum\remarks If \type T is a \mask instantiation, the function only participates in overload resolution if \type U is \bool.
  \pnum\effects If \type M is \bool, assign \code{mem[0]} to \code{data} unless \code{mask} is \false.
  If \type M is not \bool, replace the elements of \code{data} where \code{mask[i]} is \true such that the $i$-th element is assigned with \code{static_cast<typename T::value_type>(mem[i])} \foralli[M::].

  \pnum\remarks If \type M is not \bool and if the largest $i$ where \code{mask[i]} is \true is greater than the number of values pointed to by \code{mem}, the behavior is undefined.
