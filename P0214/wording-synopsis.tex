...
\pnum The \emph{selected elements} signify the elements \code{data[i]} \forallmaskedi[M::]{mask}.
...
\begin{itemdescr}
  \pnum\effects
  Replaces the selected elements of \code{data} with the corresponding elements from the application of the indicated operator on \code{data} and \code{std::forward<U>(x)}.
...
\begin{itemdescr}
  \flagsRequires{\type T, \type U}
  %\pnum\requires
  If \type M is not \bool, every i $\in$ \code{[0, M::size())} where \code{mask[i]} is \true shall be less than the number of values pointed to by \code{mem}.

  \pnum\effects Replaces the selected elements with the corresponding values \code{static_cast<value_type>(mem[i])} \forallmaskedi[M::]{mask}.
