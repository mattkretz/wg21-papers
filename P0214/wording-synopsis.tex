...
\begin{itemdescr}
  \pnum
  \type{memory_alignment<T, U>} shall have no member \code{value} unless
  \begin{itemize}
  ...
  \pnum
  Otherwise, the type \type{memory_alignment<T, U>} is a \BinaryTypeTrait with a \BaseCharacteristic of \type{integral_constant<size_t, N>} for some implementation-defined \code{N} (see \ref{sec:simd.copy} and \ref{sec:simd.mask.copy}).
  \wgNote{
...
\pnum The \emph{selected indexes} signify the integers \forallmaskedi[M::]{mask}.
The \emph{selected elements} signify the elements \code{data[i]} for all selected indexes \code{i}.
...
\begin{itemdescr}
  ...
  \pnum\effects Copies the selected elements as if \code{mem[i] = static_cast<U>(data[i])} for all selected indexes \code{i}.
  ...
  \begin{itemize}
      ...
      \item either \type U is \bool and \valuetype is \bool or
          \type U is a \realArithmeticType and \valuetype is not \bool.
  \end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\effects
  Replaces \code{data[i]} with \code{static_cast<T>(std::forward<U>(x))[i]} for all selected indexes \code i.
...
\begin{itemdescr}
  \pnum\effects
  Replaces \code{data[i]} with \code{static_cast<T>(data @ std::forward<U>(x))[i]} for all selected indexes \code i.
...
\begin{itemdescr}
  \flagsRequires{\type T, \type U}
  %\pnum\requires
  If \code{is_simd_mask_v<M>} is \true, for all selected indexes $i$, $i$ shall be less than the number of values pointed to by \code{mem}.

  \pnum\effects Replaces the selected elements as if \code{data[i] = static_cast<value_type>(mem[i])} for all selected indexes \code i.
