...
\wgSubsubsection{\mask load function}{simd.mask.load}
\begin{itemdecl}
...
\wgSubsubsection{\mask store function}{simd.mask.store}
\begin{itemdecl}
...
\begin{itemdecl}
template <class T, class Abi> int find_first_set(const simd_mask<T, Abi>& m);
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdecl}
template <class T, class Abi> int find_last_set(const simd_mask<T, Abi>& m);
\end{itemdecl}
\begin{itemdescr}
  \pnum\requires \code{any_of(m)} returns \true
...
\begin{itemdescr}
  \pnum\returns An object of type \type{const_where_expression}/\type{where_expression} (see \ref{sec:simd.whereexpr}) initialized with the predicate \code k and the value reference \code v.
\end{itemdescr}
