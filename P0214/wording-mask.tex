...
\pnum\realnote Implementations are encouraged to enable \code{static_cast}ing from/to implementation-defined SIMD mask types.
...
\begin{itemdescr}
  ...
  \pnum\returns A \mask object with each element $i$ initialized to \code{x[i]} \foralli.
  ...
  \pnum\remarks If the \type{Flags} template parameter is of type \type{flags::vector_aligned_tag} and the pointer value is not a multiple of \code{memory_alignment<\type T>}, the behavior is undefined.
\end{itemdescr}

\wgSubsubsection{\mask store functions}{mask.store}
\begin{itemdecl}
template <class T, class Abi, class Flags>
void store(const mask<T, Abi> &x, bool *y, Flags);
\end{itemdecl}
\begin{itemdescr}
  ...
  \pnum\remarks If the \type{Flags} template parameter is of type \type{flags::vector_aligned_tag} and the pointer value of \code y is not a multiple of \code{memory_alignment<\mask[<T, Abi>]>}, the behavior is undefined.
\end{itemdescr}

\begin{itemdecl}
template <class T0, class A0, class T1, class A1, class Flags>
void store(const mask<T0, A0> &x, bool *y, const mask<T1, A1> &k, Flags);
\end{itemdecl}
\begin{itemdescr}
  ...
  \pnum\remarks If the \type{Flags} template parameter is of type \type{flags::vector_aligned_tag} and the pointer value of \code y is not a multiple of \code{memory_alignment<\mask[<T0, A0>], \type U>}, the behavior is undefined.
\end{itemdescr}
