...
\begin{itemize}
  \item The specialization \type{#1<T, Abi>} is supported if \type T is a \realArithmeticType and
    \begin{itemize}
  ...
  \item If \type{Abi} is an extended ABI tag, it is implementation-defined whether \type{#1<T, Abi>} is supported.
    \wgNote{The intent is for implementations to decide on the basis of the \currentTarget.}

  \item If \type{#1<T, Abi>} is not supported, the specialization shall have a deleted default constructor, deleted destructor, deleted copy constructor, and deleted copy assignment.
\end{itemize}
}
...
\begin{itemdescr}
  ...
  \comment[Q]{\type U is cv- and ref-qualified, the wording below is not OK.}
  \begin{itemize}
...
%\pnum This subclause applies unary element-wise operations.
\begin{itemdecl}
simd& operator++();
\end{itemdecl}
\begin{itemdescr}
