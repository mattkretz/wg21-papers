...
\newcommand\dataparTypeRequirements[1]{
\pnum\label{#1.type requirements}%
...
The second template argument \type{Abi} must be an ABI tag so that \code{is_abi_tag_v<Abi>} is \true.
If the \type{Abi} type is \fixedsizeN, the value \code{N} must be supported by the implementation (see \ref{datapar.fixedsize.def} and \ref{datapar.maxfixedsize.def}).
The combination of \type T and \type{Abi} must be supported for the \currentTarget.%
\comment[Alternative]{
  It is implementation-defined whether a combination of \type T and \type{Abi} is supported.
  \wgNote{
    The intent is for implementations to decide on the basis of the \currentTarget.
  }
}
}
\dataparTypeRequirements{datapar}
...
\wgExample{
  ...
  \begin{itemize}
    \item \datapar[<T, datapar_abi::gpu_y>] is not supported for any \type T and results in a type with deleted constructor (see below)
    \item \datapar[<long double, datapar_abi::simd_x>] is not supported and results in a type with deleted constructor (see below)
    \item \datapar[<double, datapar_abi::simd_x>] is supported
    \item \datapar[<long double, datapar_abi::scalar>] is supported
  \end{itemize}
}

\newcommand\dataparDeleted[1]{
\pnum\label{#1.deleted}%
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks This constructor shall not participate in overload resolution unless \code{datapar(gen(integral_constant<size_t, 0>()))} is well-formed.%
  \comment{
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks This constructor shall not participate in overload resolution unless \type U is an arithmetic type except \bool.
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks This function shall not participate in overload resolution unless \type U is an arithmetic type except \bool.
...
\begin{itemdescr}
  ...
  \pnum\remarks \datapar{}\code{::operator\textasciitilde{}()} shall not participate in overload resolution unless \type T is an integral type.
\end{itemdescr}
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks These functions shall not participate in overload resolution unless \code{datapar_size_v<T, datapar_abi::native<T>>} is equal to \code N.
  \pnum\returns An object with the $i$-th element initialized to \code{x[i]}.
\end{itemdescr}
...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks These functions shall not participate in overload resolution unless \code{datapar_size_v<T, datapar_abi::compatible<T>>} is equal to \code N.
  \pnum\returns An object with the $i$-th element initialized to \code{x[i]}.
\end{itemdescr}
