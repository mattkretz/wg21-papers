...
\begin{itemdescr}
  ...
  \pnum\remarks If all elements in \code k are \false, the return value is unspecified.%
  \comment{Alternatively, it could return \code{numeric_limits<value_type>::max()}. Seems like an implementation detail, though.}
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\returns The value of an element $j$ for which \code{k[j] == true} and \code{operator[](j) >= operator[](i) || !k[i]} \foralli.
  \pnum\remarks If all elements in \code k are \false, the return value is unspecified.%
  \comment{Alternatively, it could return \code{numeric_limits<value_type>::min()}. Seems like an implementation detail, though.}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \begin{itemize}
    ...
    \item\label{uint conversion} Otherwise, if \type T is integral, \type U is \uint, and \type T is of smaller integer conversion rank than \intt
