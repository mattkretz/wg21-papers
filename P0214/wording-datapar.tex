...
\begin{itemdecl}
template <class U> datapar(const datapar<U, datapar_abi::fixed_size<size()>> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdecl}
template <class U, class Flags> void memstore(U *mem, Flags) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdescr}
  ...
  Additionally, \code{operator<<} and \code{operator>>} do not participate in overload resolution if \valuetype is a signed integral type.

  \pnum\returns A \datapar object initialized with the results of the component-wise application of the indicated operator.
\end{itemdescr}

\begin{itemdecl}
friend datapar operator<<(const datapar &v, int n) noexcept;
friend datapar operator>>(const datapar &v, int n) noexcept;
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks Both operators only participate in overload resolution if \valuetype is an unsigned integral type.

  \pnum\returns A \datapar object where the $i$-th element is initialized to \code{v[i] \textit{op} n} \foralli.
\end{itemdescr}
...
\begin{itemdecl}
friend datapar &operator<<=(datapar &v, int n) noexcept;
friend datapar &operator>>=(datapar &v, int n) noexcept;
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks Both operators only participate in overload resolution if \valuetype is an unsigned integral type.
...
T reduce(const datapar<T, Abi> &x, BinaryOperation binary_op = BinaryOperation()) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdecl}
template <class T, class A> T hmin(const datapar<T, A> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdecl}
template <class M, class V> T hmin(const where_expression<M, V> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
\begin{itemdecl}
template <class M, class V> T hmax(const where_expression<M, V> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
  \pnum\returns The value of an element \code{x.data[j]} for which \code{x.mask[j] == true} and \code{x.data[j] >= x.data[i]} \foralli.
...
datapar<T, /*see below*/> static_datapar_cast(const datapar<U, A> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
datapar<T, datapar_abi::native<T>> to_native(
    const datapar<T, datapar_abi::fixed_size<N>> &x) noexcept;
template <class T, size_t N>
mask<T, datapar_abi::native<T>> to_native(
    const mask<T, datapar_abi::fixed_size<N>> &x) noexcept;
\end{itemdecl}
\begin{itemdescr}
...
conditional_t<(T::size() == (U::size() + Us::size()...)), T,
            array<T, (U::size() + Us::size()...) / T::size()>> datapar_cast(U, Us...) noexcept;
\end{itemdecl}
\begin{itemdescr}
