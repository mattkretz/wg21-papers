...
\begin{itemdescr}
  ...
  \pnum\remarks If all elements in \code k are \false, the return value is \code{numeric_limits<value_type>::max()}.%
  \comment{Alternatively, it could be unspecified. The value seems like an implementation detail.}
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\returns The value of an element $j$ for which \code{k[j] == true} and \code{operator[](j) >= operator[](i) || !k[i]} \foralli.
  \pnum\remarks If all elements in \code k are \false, the return value is \code{numeric_limits<value_type>::min()}.%
  \comment{Alternatively, it could be unspecified. The value seems like an implementation detail.}
\end{itemdescr}
