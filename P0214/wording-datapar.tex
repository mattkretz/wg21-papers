...
\end{itemdecl}
\begin{itemdescr}
  \pnum\remarks Whether the function exists is implementation-defined.
...
\pnum \tab{math} summarizes the \code{<cmath>} and \code{<cstdlib>} functions that are overloaded with \datapar arguments.
\begin{table}[h]
  \rmfamily\smaller[1]
  \caption{Overloads of \code{<cmath>} and \code{<cstdlib>} functions for \datapar}
  \label{tab:math}
  \begin{tabular}[h]{|lllll|}
    ...
    \code{nanl} & \code{nearbyint} & \code{nextafter} & \code{nexttoward} & \code{remainder} \\
    \code{remquo} & \code{rint} & \code{round} & \code{scalbln} & \code{scalbn} \\
    \code{sqrt} & \code{trunc} & & & \\
    \hline
    ...
    \code{abs} & \code{div} & \code{labs} & \code{ldiv} & \code{llabs} \\
    \code{lldiv} & & & & \\
    \hline
  \end{tabular}
\end{table}
...
\pnum Each of these functions is provided for arguments of types \datapar[<float, Abi>], \datapar[<double, Abi>], and \datapar[<long double, Abi>]\comment{I would not mind dropping \type{long double} support.}, where \type{Abi} is any of the \code{datapar_abi} types supported by the implementation.
The detailed signatures are:
\lstinputlisting[]{math.cpp}

\pnum The signatures of the integer functions are:
\lstinputlisting[]{imath.cpp}
