...
\wgExample{
  Consider an implementation that defines the two additional ABI types \type{simd_x} and \type{gpu_y}.
  When the compiler is invoked to translate to a machine that has support for the \type{simd_x} ABI for all arithmetic types except \type{long double} and no support for the \type{gpu_y} ABI, then:
  \begin{itemize}
