...
\begin{itemize}
  ...
  \chck Added missing \code{const_where_expression} overload for \code{where(bool, const T\&}. (\ref{sec:simd.syn}, \ref{sec:simd_mask.where}) (The overload is present in my reference implementation.)
  \chck Removed \code{const} from \code{const_where_expression} return type. (\ref{sec:simd.syn})
  \item Fixed incorrect \code{const where_expression} return types in \ref{sec:simd_mask.where}.
  ...
  \todo Every time I read A I think Allocator: can the be Abi instead?
  \todo stable names should follow a hierarchy; for [simd] I would suggest [simd.class]; so everything should be [simd.*]
