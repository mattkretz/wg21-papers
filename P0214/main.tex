...
\begin{itemize}
  ...
  \chck Clarify effects of using an ABI parameter that is not available on the target (\ref{datapar.deleted}, \ref{mask.deleted}, \ref{datapar_size}).
  \chck Split \whereexpression into \const and non-\const class templates.
  \chck Add section on naming (\ref{sec:Naming}).
  \chck Remove non-normative noise about ABI tag types (\ref{sec:datapar.abi}).
  \chck Remove most of the text about vendor-extensions for ABI tag types, since it's QoI (\ref{sec:datapar.abi}).
  \chck Clarify the differences and intent of \type{compatible<T>} vs. \type{native<T>} (\ref{sec:datapar.abi}).
  \chck Move definition of \whereexpression out of the synopsis (\ref{sec:datapar.whereexpr}).
  \chck Change “implementation_defined” to “implementation-defined”.
  \chck Discuss the questions/issues raised on \code{max_fixed_size} in Kona (\ref{sec:maxfixedsize}).
  \chck Make \code{max_fixed_size} dependent on \type{T}.
