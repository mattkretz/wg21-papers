...
\newcommand\flagsRequires[1]{%
  \pnum\requires
  If the template parameter \type{Flags} is \type{vector\_aligned\_tag}, the pointer value shall represent an address aligned to \code{memory\_alignment\_v<#1>}.
  If the template parameter \type{Flags} is \type{overaligned\_tag<N>}, the pointer value shall represent an address aligned to \code N.
}
...
\begin{itemize}
  ...
  \todo Introduce exposition-only \code{nodeduce_t} to replace \code{remove_const_t}.
