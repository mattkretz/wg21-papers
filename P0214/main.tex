...
\newcommand\flagsRemarks[1]{
  \pnum\remarks If the \type{Flags} template parameter is of type \type{flags::vector\_aligned\_tag} and the pointer value is not a multiple of \code{memory\_alignment<#1>}, the behavior is undefined.
  If the \type{Flags} template parameter is of type \type{flags::overaligned\_tag<N>} and the pointer value is not a multiple of \code N, the behavior is undefined.
}
...
\begin{itemize}
    ...
    \chck Restricted broadcasts to only allow non-narrowing conversions (\ref{sec:datapar.ctor}).
    \chck Restricted datapar to datapar conversions to only allow non-narrowing conversions with \type{fixed_size} ABI (\ref{sec:datapar.ctor}).
    \chck Added generator constructor (as discussed in LEWG in Issaquah) (\ref{sec:datapar.ctor}).
    \chck Renamed \code{copy_from} to \code{memload} and \code{copy_to} to \code{memstore}. (\ref{sec:datapar.load}, \ref{sec:datapar.store}, \ref{sec:mask.load}, \ref{sec:mask.store})
    \chck Documented effect of \type{overaligned_tag<N>} as \type{Flags} parameter to load/store. (\ref{sec:datapar.load}, \ref{sec:datapar.store}, \ref{sec:mask.load}, \ref{sec:mask.store})
    \chck Clarified requirements on \type T parameter of \datapar and \mask.
    \chck Allowed all implicit \mask conversions with \fixedsize ABI and equal size (\ref{sec:mask.ctor}).
    \chck Made increment and decrement of \type{where_expression} return \type{void}.
    \chck Added \code{static_datapar_cast} for simple casts (\ref{sec:datapar.casts}).
    \chck Clarified default constructor (\ref{sec:datapar.overview}, \ref{sec:datapar.overview}).
    \chck Clarified \datapar and \mask with invalid template parameters to be complete types with deleted constructors, destructor, and assignment (\ref{sec:datapar.overview}, \ref{sec:datapar.overview}).
    \chck Wrote a new subsection for a detailed description of \type{where_expression} (\ref{sec:datapar.whereexpr}).
    \chck Moved masked loads and stores from \datapar and \mask to \type{where_expression} (\ref{sec:datapar.whereexpr}).
          This required two more overloads of \code{where} to support value objects of type \mask (\ref{sec:mask.where}).
    \chck Removed \type{where_expression}\code{::operator!} (\ref{sec:datapar.whereexpr}).
    \chck Added aliases \type{native_datapar}, \type{native_mask}, \type{fixed_size_datapar}, \type{fixed_size_mask} (\ref{sec:datapar.syn}).
    \chck Removed \bool overloads of mask reductions awaiting a better solution (\ref{sec:mask.reductions}).
    \chck Removed special math functions with \code f and \code l suffix and \code l and \code{ll} prefix (\ref{sec:datapar.math}).
    \chck Modified special math functions with mixed types to use \type{fixed_size} instead of \code{abi_for_size} (\ref{sec:datapar.math}).
    \chck Added simple ABI cast functions \code{to_fixed_size}, \code{to_native}, and \code{to_compatible} (\ref{sec:datapar.casts}).
\end{itemize}
