...
\newcommand\wgDocumentNumber{D0214R7}
...
\newcommand\flagsRemarks[1]{
  \pnum\requires
  If the \type{Flags} template parameter is of type \type{vector\_aligned\_tag}, the pointer value represents an address aligned to \code{memory\_alignment\_v<#1>}.
  If the \type{Flags} template parameter is of type \type{overaligned\_tag<N>}, the pointer value represents an address aligned to \code N.
}
...
\begin{itemize}
  ...
  \todo you copy initialze some value but direct initialize others: they should be consistent
  \todo for the see belows we don't put comments but use italics
  \todo all of the casts can benefit from the aliases or the default args
  \todo why are the arguments by value? Consistently pass arguments by const-ref or by value
  \todo reduce: the defaulted argument should be moved right to the end
  \todo reduce: define \code{default_natural_element} (or mark it see-below / implementation-defined)
\end{itemize}

\subsection{Changes from R5}
Previous revision: \parencite{P0214R5}.
\begin{itemize}
  \item Renamed \code{memload} to \code{copy_from} and \code{memstore} to \code{copy_to}. (\ref{sec:simd.load}, \ref{sec:simd.store}, \ref{sec:simd.mask.load}, \ref{sec:simd.mask.store})
...
\begin{itemize}
    ...
    \item Renamed \code{copy_from} to \code{memload} and \code{copy_to} to \code{memstore}. (\ref{sec:simd.load}, \ref{sec:simd.store}, \ref{sec:simd.mask.load}, \ref{sec:simd.mask.store})
    \item Documented effect of \type{overaligned_tag<N>} as \type{Flags} parameter to load/store. (\ref{sec:simd.load}, \ref{sec:simd.store}, \ref{sec:simd.mask.load}, \ref{sec:simd.mask.store})
...
\begin{itemize}
  ...
  \color{black}\item[---] LEWG (small group) guidance:
    ...
    Instead have \code{copy_from} and \code{copy_to} member functions for loads and stores. (\ref{sec:simd.load}, \ref{sec:simd.store}, \ref{sec:simd.mask.load}, \ref{sec:simd.mask.store})
