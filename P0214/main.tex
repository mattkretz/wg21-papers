...
\begin{itemize}
    ...
    \chck Clarify requirements on \type T parameter of \datapar and \mask.
    \chck Allow all implicit \mask conversions with \fixedsize ABI and equal size (\ref{sec:mask.ctor}).
    \chck Increment and decrement of \type{where_expression} return \type{void}.
    \chck Added \code{static_datapar_cast} for simple casts (\ref{sec:datapar.casts}).
    \chck Clarify default constructor (\ref{sec:datapar.overview}, \ref{sec:datapar.overview}).
\end{itemize}
