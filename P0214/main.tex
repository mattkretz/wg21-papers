...
\begin{itemize}
    \chck Fixed return type of masked \code{reduce}.
    \chck Fixed return type of binary operators if the second type is \uint (\ref{uint conversion}).
    \chck Fixed neutral element of masked \code{min}/\code{max} and drop UB.
    \chck Added binary \code{min}, \code{max}, \code{minmax}, and \code{clamp}.
    \chck Removed reduction member functions in favor of non-member \code{reduce} (as requested by LEWG).
    \chck Replaced \code{init} parameter of masked \code{reduce} with \code{neutral_element}.
    \chck Added non-member \code{hmin} and \code{hmax}, which cannot easily be optimized from \code{reduce}, since no function object such as \code{std::plus} exists.
    \chck Extend \type{where_expression} to support \const \datapar objects.
    \chck Fixed \code{where} wording to not ignore \type{where_expression}.
    \chck Made \code{mask(bool)} constructor explicit.
    \chck Made binary operators for \datapar and \mask friend functions of \datapar and \mask, simplifying the SFINAE requirements considerably.
    \chck Restrict broadcasts to only allow non-narrowing conversions.
    \chck Restrict datapar to datapar conversions to only allow non-narrowing conversions with \type{fixed_size} ABI.
