...
\begin{itemize}
    ...
    \chck Modified the wording of \code{\mask{}::native_handle()} to make the existence of the functions implementation-defined.
    \chck Updated the discussion of member types to reflect the changes in R1.
    \chck Added all previous SG1 straw poll results.
    \chck Fixed \code{\textit{commonabi}} to not invent native Abi that makes the operator ill-formed.
    \chck Dropped table of math functions.
    \chck Be more explicit about the implementation-defined Abi types.
    \chck Discussed resolution of the \fixedsizeN design (\ref{sec:fixedsize progress}).
    \chck Made the \type{compatible} and \type{native} ABI aliases depend on \type T (\ref{sec:datapar.abi}).
    \chck Added \code{max_fixed_size} constant (\ref{sec:datapar.maxfixedsize.def}).
    \chck Added masked loads.
  \color{black}\item[---] SG1 guidance:
    \chck Dropped the default load / store flags.
    \chck Renamed the (un)aligned flags to \code{element_aligned} and \code{vector_aligned}.
    \chck Added an \code{overaligned<N>} load / store flag.
    \chck Dropped the ampersand on \code{native_handle} (no strong preference).
    \chck Completed the set of math functions (i.e. add trig, log, and exp).
  \color{black}\item[---] LEWG guidance:
    \chck Dropped \code{native_handle} and add non-normative wording for supporting \code{static_cast} to implementation-defined SIMD extensions.
    \chck Dropped non-member load and store functions.
    ...
    See how \code{where} could be integrated properly to do masked reductions.
    \todo Consider \code{cardinality} instead of \code{popcount} (or different bikeshed colors).
\end{itemize}
