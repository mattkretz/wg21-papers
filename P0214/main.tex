...
\begin{itemize}
  ...
  \item Discuss the questions/issues raised on \code{max_fixed_size} in Kona (\ref{sec:maxfixedsize}).
  ...
  \item Discuss \mask and \type{bitset} reduction interface differences (\ref{sec:mask queries}).
  ...
  \item Add proposal for \code{split} and \code{concat} cast functions (\ref{sec:split and concat}).
