...
\begin{itemize}
    ...
    \chck Moved member \code{min} and \code{max} to non-member \code{hmin} and \code{hmax}, which cannot easily be optimized from \code{reduce}, since no function object such as \code{std::plus} exists (\ref{sec:datapar.reductions}).
    \chck Fixed neutral element of masked \code{hmin}/\code{hmax} and drop UB (\ref{sec:datapar.reductions}).
    \chck Removed remaining reduction member functions in favor of non-member \code{reduce} (as requested by LEWG).
    ...
    \chck Fixed missing \code{explicit} keyword on \code{mask(bool)} constructor (\ref{sec:mask.ctor}).
    ...
    \chck Clarified cv requirements on \type T parameter of \datapar and \mask.
