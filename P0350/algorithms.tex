...
@\wgAdd{class \simd{}_policy \{ \textit{unspecified} \};}@
  ...
  \begin{itemdescr}
    \wgAdd{
      %\setcounter{Paras}{1}
      \pnum The class \simd\type{_policy} is an execution policy type used as a unique type to disambiguate parallel algorithm overloading and indicate that a parallel algorithm's execution may be vectorized using \simd{} for interfacing with user-provided functionality.
