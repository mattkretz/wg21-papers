...
\begin{itemize}
  ...
  \item \code{generate}, \code{generate_n}

    Note that the generator function passed to \code{generate}/\code{generate_n} does not take any arguments and thus has no interface for the algorithm to request a certain ABI tag from the function (template).
    Consequently, either the user could choose the ABI tag (via the return type) and expect values at the tail (beyond \code{end}) to be discarded.
    ...
    See \fig{generate example} for an example.
    It is possible to allow both variants.
  \item \code{remove}, \code{remove_if}, \code{remove_copy}, \code{remove_copy_if}
...
I have not considered \code{is_heap} and \code{is_heap_until} yet.
