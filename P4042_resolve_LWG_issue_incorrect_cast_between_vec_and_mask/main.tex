\newcommand\wgTitle{Fix LWG4543: incorrect cast between \code{simd::vec} and \code{simd::mask} via conversion to and from impl-defined vector types}
\newcommand\wgName{Matthias Kretz <m.kretz@gsi.de>}
\newcommand\wgDocumentNumber{P4042R0}
...
\begin{wgTitlepage}
  This paper resolves \lwg{4543} and includes a drive-by fix to [simd.mask.conv].
\end{wgTitlepage}
...
\begin{codeblock}
    ...
    @\wgAdd{template<class U, class UAbi>}@
      @\wgAdd{constexpr explicit(\seebelow) basic_vec(const basic_vec<U, UAbi>\&) noexcept = delete;}@
    template<class G>
...
\pnumAdd\wgAdd{%
\remarks
The expression inside \tcode{explicit} evaluates to \tcode{\exposid{simd-size-v}<U, UAbi> ==
size()}.
}
\end{itemdescr}
\end{wgText}
...
\begin{codeblock}
    ...
    @\wgAdd{template<size_t UBytes, class UAbi>}@
      @\wgAdd{constexpr explicit basic_mask(const basic_mask<UBytes, UAbi>\&) noexcept = delete;}@
    @\wgAdd{template<class U, class UAbi>}@
      @\wgAdd{constexpr explicit basic_mask(const basic_vec<U, UAbi>\&) noexcept = delete;}@
    template<class G>
    ...
    @\wgAdd{template<class U, class UAbi>}@
      @\wgAdd{constexpr operator basic_vec<U, UAbi>() const noexcept = delete;}@
...
\begin{itemdecl}
@\wgAdd{template<size_t UBytes, class UAbi>}@
  @\wgAdd{constexpr explicit basic_mask(const basic_mask<UBytes, UAbi>\& x) noexcept = delete;}@
\end{itemdecl}
