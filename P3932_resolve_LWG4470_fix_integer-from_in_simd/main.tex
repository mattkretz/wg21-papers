...
\newcommand\wgDocumentNumber{P3932R0}
...
\begin{codeblock}
  ...
  template<class T, @\exposid{simd-size-type}@ N = @\exposid{simd-size-v}@<T, @\exposid{native-abi}@<T>>>
    using mask = @\wgChange{basic_mask<sizeof(T), \mbox{\exposid{deduce-abi-t}}<T, N>\mbox{}>}{vec<T, N>::mask_type}@;
\end{codeblock}
\end{wgText}
...
\pnum
Where present, the member typedef \tcode{type} names \tcode{basic_vec<\wgChange{T}{typename
V::value_type}, Abi1>}
if \tcode V is a specialization of \tcode{basic_vec} or
...
\begin{itemize}
 \item
    ...
    \tcode{N} in the range \crange{1}{64}, such that \tcode{Abi} \wgChange{is}{names the ABI tag
    type denoted by} \tcode{\exposid{deduce-abi-t}<T, N>},
 \item
...
\begin{itemize}
  ...
  \code{basic_mask}, and]
  \wgItemAdd[\tcode{basic_vec<T, Abi>::size()} is equal to \tcode{basic_vec<T, Abi>::mask_type::size()}].
\end{itemize}
...
\begin{itemize}
 ...
 \item
   ...
   to \tcode{sizeof(T)} and \tcode{Abi} \wgChange{is}{names the ABI tag type denoted by}
