...
\begin{figure}[]
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (Mark5.west) -- +(-3.0\vNodeWidth,0) |- (G0);
    \draw[->] (Mark6.west) -- +(-3.2\vNodeWidth,0) |- (J0);
    ...
    \draw[<-] (Mark5.east) -- +(3.0\vNodeWidth,0) |- (I0);
    \draw[<-] (Mark6.east) -- +(3.2\vNodeWidth,0) |- (L0);
  \end{tikzpicture}
  \caption{Visualization of chunking the \code{foreach} call in \lst{datapar foreach}.}
