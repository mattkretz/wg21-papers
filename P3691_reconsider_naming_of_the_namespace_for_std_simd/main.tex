...
\newcommand\wgDocumentNumber{P3691R0}
...
The outcome of that poll should inform us where \code{std::simd} can or cannot go.

If LEWG decides to ban name repetition between namespace and enclosed type, then we need to reconsider
whether \stddatapar\code{simd} / \stddatapar\code{simd_mask} is really still better than
\stdsimd\code{pack}\footnote{or one of the other alternatives mentioned; to be discussed on the
reflector} / \stdsimd\code{mask}.
