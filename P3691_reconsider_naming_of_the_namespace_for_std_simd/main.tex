...
\begin{wgTitlepage}
  ...
  and recommends to rename the namespace to \code{simd} and subsequently drop the \code{simd_}
  part from \code{basic_simd_mask} and \code{simd_mask}.
\end{wgTitlepage}
...
\section{Criticism and discussion on the current name}
...
Then the issue is that “data-parallel SIMD” sounds strange because SIMD that is not data-parallel
doesn't exist.
...
However, the intent was for \code{datapar} to be an abbreviation of “Data-parallel types”, the
subclause heading in the current working draft.
...
template name but alias template.
...
We believe the fear, uncertainty, and doubts about repeating the name of the namespace in the alias
template are possibly no more than that; and in reality the “problem” is little to non-existent.
...
\subsection{Rust's experimental std::simd}
...
\begin{tabular}{l|l}
  ...
  \optionrow{datapar}{simd}{repetitive in a different way}
  \optionrow{dataparallel}{simd}{ditto}
...
If we were to use a different name for the namespace and class/alias templates then it is probably
better to use the “SIMD” name for the class/alias name rather than the namespace.
The class/alias name is the identifier that will always appear in the source code so choosing a
class name which is ambiguous once the namespace is removed (e.g., with a using statement) obscures
the codes intent.
We would end up with variables with types like \code{value<int>}, \code{number<int>},
\code{pack<int>}, and so on, and all hint of their SIMD behaviour is lost.
...
\end{lstlisting}
On the contrary, naming the namespace of the \code{reduce} function after the type reinforces the
