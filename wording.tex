...
namespace datapar_abi {
  ...
  \begin{itemdescr}
    ...
    \pnum
    The \type{datapar_abi::widest} tag is defined by the implementation to alias the tag type with the most efficient data parallel execution that is supported on the target system.
  \end{itemdescr}
...
datapar_return_type<datapar<T, Abi>, U> operator>>(const U &, datapar<T, Abi>);
  ...
  \begin{itemdescr}
    \pnum\remarks The return type of these operators shall be deduced according to the following rules:
       \begin{itemize}
         \item If \code{is_datapar_v<U> == true}
           then the return type shall be \datapar{}\type{<decltype(declval<T>() + declval<typename U::value_type>()), Abi>}.
         \item Otherwise, if \type U is a fundamental arithmetic type and \datapar{}\type{<U, Abi>} is a valid type
           then the return type shall be \datapar{}\type{<decltype(declval<T>() + declval<U>()), Abi>}.
         \item Otherwise, if \type U is implicitly convertible to \datapar{}\type{<V, Abi>}, where \type{V} is determined according to standard template type deduction,
           then the return type shall be \datapar{}\type{<decltype(declval<T>() + declval<V>()), Abi>}.
       \end{itemize}
    ...
    \pnum\returns An object of type \type{datapar_return_type<datapar<T, Abi>, U>} containing the results of the component-wise operator application.
  \end{itemdescr}
...
template <class T, class Abi, class U> datapar<T, Abi> &operator>>=(datapar<T, Abi> &, U &&);
  ...
  \begin{itemdescr}
    ...
    \begin{itemize}
      ...
      \item \type U is implicitly convertible to \type{datapar_return_type<datapar<T, Abi>, U>}.
    \end{itemize}
    \pnum\effects Each of these operators performs the indicated operation component-wise on each the elements of the first argument and the corresponding element of the second argument after conversion to \datapar{}\code{<T, Abi>}.
    \pnum\notes   The conversion of the second argument to \datapar{}\code{<T, Abi>} perfectly forwards the second argument.
