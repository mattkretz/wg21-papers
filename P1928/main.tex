...
\begin{enumerate}
  \item\label{enum:reduce_min} \code{reduce_min(simd)} and
    \code{reduce_max(simd)}
    \begin{description}
  ...
  \item\label{enum:min_element} \code{min_element(simd)} and
    \code{max_element(simd)}
    \begin{description}
  ...
  \item\label{enum:reduce_min_element} \code{reduce_min_element(simd)} and
    \code{reduce_max_element(simd)}
    \begin{description}
  ...
  \item\label{enum:horizontal_min} \code{horizontal_min(simd)} and
    \code{horizontal_max(simd)}
    \begin{description}
      \item[Pro] States that a horizontal operation is applied.
      \item[Con] We don't use the term „horizontal'' anywhere else.
    \end{description}

  \item\label{enum:extract_min} \code{extract_min(simd)} and
    \code{extract_max(simd)} (\emph{my recommendation --- unless convinced
    otherwise})
    \begin{description}
      \item[Pro] The term „extract'' states that a single element will be
        extracted. Likely a good matching name to the \code{extract} facility
        that we might add later.
      \item[Con] Inconsistent with \std\code{reduce(simd)} (maybe okay because
        slightly different: \code{reduce(simd)} returns an aggregate of all
        elements in the \simd, \code{extract_max} „searches'' for a certain
...
\wgPoll{Rename to \code{extract_min} and \code{extract_max}.}
{&&&&}
