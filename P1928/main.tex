...
\begin{itemize}
  \item \code{auto conditional_operator(simd_mask, \textit{simd_like},
    \textit{simd_like})}\footnote{to be replaced by an actual \code{operator?:}
...
\begin{lstlisting}[numbers=left,float={hbtp},label=lst:mixedsimdandmask,caption={
    Binary operations involving \code{simd_mask} and \code{simd}
}]
using floatv = std::simd<float>;
using intv = std::rebind_simd_t<int, floatv>;

int count_positive(const std::vector<float>& x)
{
  intv counter = {};
  for (std::size_t i = 0; i + floatv::size <= x.size(); i += floatv::size) {
    counter += floatv(&x[i]) > 0;
  counter += x > 0;
}
\end{lstlisting}
