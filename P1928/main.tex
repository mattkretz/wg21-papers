...
\begin{itemize}
  ...
  \item \code{simd_mask::operator\~{}() const noexcept}
  \item \code{simd_mask::operator?:() const noexcept}
  \item \code{onstexpr simd_mask operator?:(const simd_mask\&, const simd_mask\&, const simd_mask\&) noexcept}
  \item \code{onstexpr simd_mask operator?:(const simd_mask\&, bool, bool) noexcept}
  \item \code{operator?:(const simd_mask\&, const T0\&, const T1\&) noexcept}
\end{itemize}
...
Is there a use case for \code{std::hash<simd<T>>}?
...
\code{std::hash<simd<T>>} the right interface to compute hashes element-wise?
...
\begin{enumerate}
  ...
  \item\label{enum:valuepreserving} Stick to the \emph{value-preserving} rules with exceptions for
    \code{int} and \code{unsigned int} as introduced in the TS.
...
explicit via \code{explicit(not std::same_as<nopromo_common_type_t<T, U>, T>)}.
\code{nopromo_common_type_t} would be \code{common_type_t} but without integral
promotions.
...
With choice \ref{enum:valuepreserving} the \code{simd<int> * simd<float>} and \code{simd<long
long> * simd<float>} cases are ill-formed.
...
Then, \code{std::simd<short> x = std::cc<1>;} is well-formed while
\code{std::simd<short> x = 1;} is not.%
\footnote{Then we only need a language feature to turn constant expressions into \code{integral_constant}/\code{constexpr_t}/etc. via opt-in on the constructor/function argument and \code{std::cc} becomes transparent.}

\subsubsection{Suggested Polls}

\wgPoll{Base \simd conversion rules on the non-promoting common-type method instead of value-preserving conversions.}
{&&&&}

\wgPoll{Remove broadcast ctor exceptions for \code{int} and \code{unsigned int}, instead ensure \code{integral_constant}-like arguments work correctly.}
{&&&&}
...
\begin{enumerate}
  ...
  \item Add a \std\code{conditional_operator(cond, a, b)} CPO that tries the following:
    \begin{itemize}
      ...
      \item if \code{std::common_type_t<decltype(a), decltype(b)>} exists, cast \code{a} and \code{b} to this common type and apply \code{?:}.
    \end{itemize}
\end{enumerate}
