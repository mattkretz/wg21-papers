...
\newcommand\wgDocumentNumber{P1928R15}
\newcommand\wgGroup{LWG}
\newcommand\wgTarget{\CC{}26}
\newcommand\wgAcknowledgements{Thanks to Tomasz Kamiński and Christian Trott
for a lot of wording help. Thanks to Daniel Towner, Ruslan Arutyunyan, Jonathan
Müller, Jeff Garland, and Nicolas Morales for discussions and/or pull requests
on this/previous paper(s). Thanks to the LWG group for a lot of good review.}
...
\begin{itemize}
  ...
  \item At the Varna LEWG meeting it was decided to remove the \code{simd_abi}
    ...
    hide ABI tags.
    Consequently, \std\code{simd<T, N = \UNSP{native-size}>} is an alias for
    \std\code{basic_simd<T, Abi>} now.
\end{itemize}
...
\begin{itemize}
  \item Analogue semantics to \code{?:}, but without lazy evaluation.
...
Different to the TS, this paper uses \code{\simdsizetype} instead of \code{size_t} for
\begin{itemize}
...
Since \code{<bit>} functions like \code{std::popcount}, \code{std::bit_width},
\code{std::countl_zero}, \ldots return \code{int}, the natural choice is to
...
Therefore, both \code{operator[]} overloads are now lvalue-ref qualified to
make \code{(x * 2)[0]} pick the const overload, which returns a prvalue that is
not assignable.
...
after \CC{}26.
The changes relative to the TS are shown in \tabref{tab:loadstorenames}.
...
The trait \stdx\code{is_simd_flag_type} has been removed because the flag
parameter is now constrained via the \code{simd_flags} class template.
