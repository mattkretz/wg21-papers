...
\begin{itemize}
  ...
  \item hidden friend \code{operator?:}\footnote{to be replaced by an actual
    \code{operator?:} overload as soon as EWG lifts the restriction\ldots} /
    \code{conditional_operator} functions in \code{simd} and \code{simd_mask}:
    \begin{itemize}
  ...
  \item facilities for converting \code{simd_mask<T>} to \code{simd<T>} with
    ...
    \begin{itemize}
      \item unary \code{simd_mask::operator+}
      \item unary \code{simd_mask::operator-}
...
and maximum.
Since the merge of Ranges (\CC{}20), we have \std\code{ranges::min} and
\std\code{ranges::max}.
...
This doesn't work with \code{std::ranges::min} (and \code{max}) because it
requires an lvalue reference as return type.
If we added another \code{operator()} to \code{std::ranges::min}, then their
...
then the
\std\code{ranges::min(\MayBreak{}ranges\MayBreak{}::\MayBreak{}input_range
auto\&\&, ...)} overload matches and \std\code{ranges::min(simd)} works out of
the box.
...
\wgPoll{Extend \code{std::ranges::min} and \code{max} to allow prvalue return types.}
{&&&&}
...
The functions in \code{<bit>} were added in \CC{}20 and now \code{simd_mask}
...
\begin{itemize}
  \item \code{popcount(simd_mask)} and \code{popcount(}\textit{bool}\code{)}:\\
    ...
    The TS merge therefore will add \code{bool} to the list of valid types for
    \code{popcount}.
...
Renaming the function is not obvious though.
