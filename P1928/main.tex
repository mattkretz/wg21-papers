...
\begin{itemize}
  ...
  \item \code{simd<nonpromoting_common_type_t<T0, T1> \simdselect(const basic_simd_mask\&, const T0\&, const T1\&) noexcept}
\end{itemize}

The following operator was added to \simd:
\begin{itemize}
  \item \code{operator?:(const mask_type\& mask, const basic_simd\& a, const basic_simd\& b) noexcept}
\end{itemize}
...
Rationale: AVX-512 \code{simd<char>::size() == 64}.

\wgPoll{Increase the minimum supported \code{simd} width to 64}
{&&&&}
