...
\subsection{Casts}\label{sec:casts}
...
\subsubsection{named cast functions}
...
However, from my experience it is better to enable implicit conversions between
\code{native<T>} and \code{fixed_size<T, N>} with equal element count even if
such conversion might be non-portable between targets with different native
SIMD widths.
...
There is no way to know how \code{N} was determined, so we have to design for
one assumption or the other.
Since it should be recommended practice for users to compile \code{simd} code
to at least two different targets with different SIMD widths for CI,
...
\subsubsection{(explicit) converting constructors}
...
According to the above points the proposed converting constructor only has a
constraint on the number of elements being equal.
However, it is conditionally \code{explicit}:
Implicit casts are only allowed if one of the types uses the \code{fixed_size}
ABI tag and the \code{value_type} difference is such that all values are
preserved (same wording as in the TS).
...
completely invisible as shown in \lst{mixedprecision}.
...
Allow implicit conversions if one of the types has a \code{fixed_size} ABI tag
and the \code{sizeof} the element types are equal, otherwise the conversion
must be explicit.
...
function proposed by \textcite[8]{P2638R0}.
...
\begin{enumerate}
  ...
  \item \code{simd<T0, A0>} is implicitly convertible to \code{simd<T1, A1>} if, in addition,
    \begin{itemize}
      \item the conversion \code{T0} to \code{T1} is value-preserving, and
      \item either \code{A0} or \code{A1} is a specialization of \code{fixed_size}.
    \end{itemize}

  \item \code{simd_mask<T0, A0>} is convertible to \code{simd_mask<T1, A1>} if
    \code{simd_size_v<T0, A0> == simd_size_v<T1, A1>}.

  \item \code{simd_mask<T0, A0>} is implicitly convertible to \code{simd_mask<T1, A1>} if, in addition,
    \begin{itemize}
      \item \code{sizeof(T0) == sizeof(T1)}, and
      \item either \code{A0} or \code{A1} is a specialization of \code{fixed_size}.
    \end{itemize}

  \item \code{simd<T0, A0>} can be \code{bit_cast}ed to \code{simd<T1, A1>} if
    \code{sizeof(simd<T0, A0>) == sizeof(simd<T1, A1>)}.
    I.e. all \code{simd} specializations are trivially copyable.
  ...
  \item \code{simd_mask<T0, A0>} can be \code{bit_cast}ed to \code{simd_mask<T1, A1>} if
    ...
    I.e. all \code{simd_mask} specializations are trivially copyable.

  \item \std\code{simd_cast<T1>(x)} is basically a shorthand for
    \code{static_cast<std::rebind_simd_t<T1, decltype(x)>>(x)}.

  \item \std\code{simd_bit_cast<T1>(x)} is basically a shorthand for
    \code{std::bit_cast<std::rebind_simd_t<T1, decltype(x)>>(x)}.
\end{enumerate}
...
See Section \ref{sec:casts} for a discussion.
