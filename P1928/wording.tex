...
namespace std@\wgRem{::experimental \{ inline namespace parallelism_v2}@ {
  namespace simd_abi {
    using scalar = @\seebelow@;@\diffbreak{4}%
    \wgRem{template<int N> using fixed_size = \mbox{\seebelow};}@
    ...
    @\wgAdd{template<class T> using compatible = \impdef;}@
    @\wgAdd{template<int N> using fixed_native = \seebelow;}@
    @\wgAdd{template<int N> using fixed_compatible = \seebelow;}\diffbreak{0}
    \diffbreak{4}
    \wgRem{template<class T, size_t N, class... Abis> struct deduce \{ using type = \seebelow; \};}\diffbreak{4}
    \wgRem{template<class T, size_t N, class... Abis> using deduce_t =}\diffbreak{6}
      \wgRem{typename deduce<T, N, Abis...>::type;}@
  }
  ...
  template<class T, int N> using @\wgChange{fixed_size_simd}{fixed_native_simd} =\diffbreak{3}@ simd<T, simd_abi::@\wgChange{fixed_size}{fixed_native}@<N>>;
  @\wgAdd{template<class T> using compatible_simd = simd<T, simd_abi::compatible<T>>;}@
  @\wgAdd{template<class T, int N> using fixed_compatible_simd = simd<T, simd_abi::fixed_compatible<N>>;}@
  ...
  template<class T, int N> using @\wgChange{fixed_size_simd_mask}{fixed_native_simd_mask}@ =
    simd_mask<T, simd_abi::@\wgChange{fixed_size}{fixed_native}@<N>>;
  @\wgAdd{template<class T> using compatible_simd_mask = simd_mask<T, simd_abi::compatible<T>>;}@
  @\wgAdd{template<class T, int N> using fixed_compatible_simd_mask =}@
    @\wgAdd{simd_mask<T, simd_abi::fixed_compatible<N>>;}@
...
\begin{codeblock}
namespace simd_abi {
  using scalar = @\seebelow@;@\diffbreak{2}
  \wgRem{template<int N> using fixed_size = \seebelow;}@
...
\begin{itemdescr}
  ...
  \begin{itemize}
    ...
    \item if both \tcode{U} and \tcode{value_type} are integral, the integer conversion rank (\ref{conv.rank}) of \tcode{value_type} is greater than the integer conversion rank of \tcode{U}.
  \end{itemize}
\end{itemdescr}
