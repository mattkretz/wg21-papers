...
\begin{itemize}
  \item all possible values of \tcode{U} can be represented with type \tcode{T}, and
  \item if both \tcode{U} and \tcode{T} are integral types, the integer conversion rank (\iref{conv.rank}) of \tcode{U} is greater than the integer conversion rank of \tcode{T}, and
  \item if both \tcode{U} and \tcode{T} are floating-point types, the floating-point conversion rank (\iref{conv.rank}) of \tcode{U} is greater than the floating-point conversion rank of \tcode{T}.
\end{itemize}
...
\begin{itemdescr}
  ...
  \pnum\remarks
  The constructor is \tcode{explicit} iff the conversion from \tcode{U} to
  \tcode{value_type} is not value-preserving.
\end{itemdescr}
