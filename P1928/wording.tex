...
public:
  ...
  @\wgAdd{constexpr explicit(\seebelow) }@simd_mask(const simd_mask<U, @\wgChange{simd_abi::fixed_size<size()>}{UAbi}@>&) noexcept;
...
\begin{itemdecl}
template<class U> @\wgAdd{constexpr }@simd_mask(const simd_mask<U, simd_abi::fixed_size<size()>>& x) noexcept;
\end{itemdecl}
...
\begin{wgBAdd}
\begin{itemdecl}
@\wgAdd{template<class G> constexpr simd_mask(G\&\& gen) noexcept;}@
\end{itemdecl}
