...
namespace std {
  ...
  template<size_t Bytes, class Abi> constexpr int reduce_count(const basic_simd_mask<Bytes, Abi>&) noexcept;
  template<size_t Bytes, class Abi> constexpr int reduce_min_index(const basic_simd_mask<Bytes, Abi>&);
  template<size_t Bytes, class Abi> constexpr int reduce_max_index(const basic_simd_mask<Bytes, Abi>&);
...
\pnum
TODO: \deducet<T, N> must produce a valid ABI tag at least for N in range [1, 32].

\pnum
\tcode{\deducet<T, N>} is an alias for an unspecified ABI tag.
Use of the \tcode{\deducet<T, N>} tag type requires data-parallel types to
store \tcode{N} elements (i.e. \tcode{basic_simd<T, \deducet<T, N>>::size} equals
\tcode{N}).
\tcode{basic_simd<T, \deducet<T, N>>} and \tcode{basic_simd_mask<T, \deducet<T,
N>>} with \tcode{N > 0} and \tcode{N <= 32} shall be supported.
%Additionally, for every supported \tcode{basic_simd<T, Abi>} (see
%\ref{simd.overview}), where \tcode{Abi} is an ABI tag that is not a
%specialization of \tcode{simd_abi::fixed_size}, \tcode{N == basic_simd<T,
...
\begin{itemize}
  \item \tcode{basic_simd<T, type>::size == N}, and
  \item \tcode{basic_simd<T, type>} is default constructible (see \ref{simd.overview}).
\end{itemize}
...
\begin{itemdescr}
  ...
  \tcode{basic_simd_mask<sizeof(T), Abi1>} if \tcode V is \tcode{basic_simd_mask<U, Abi0>}.
\end{itemdescr}
...
\begin{itemdescr}
    \pnum \tcode{size} is an \tcode{integral_constant<size_t, N>} with \tcode{N} equal to the number of elements in a \tcode{basic_simd} object.
...
\begin{itemdecl}
template<size_t Bytes, class Abi> constexpr int reduce_count(const basic_simd_mask<Bytes, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<size_t Bytes, class Abi> constexpr int reduce_min_index(const basic_simd_mask<Bytes, Abi>& k);
\end{itemdecl}
...
\begin{itemdecl}
template<size_t Bytes, class Abi> constexpr int reduce_max_index(const basic_simd_mask<Bytes, Abi>& k);
\end{itemdecl}
