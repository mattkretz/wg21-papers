...
\end{itemdecl}
\begin{itemdescr}
  \pnum Let \tcode{x} denote a \tcode{const} lvalue reference of type \tcode{T}.
...
\begin{itemdescr}
  \pnum
  Let \tcode{T0} denote the first type in \tcode{Ts}.
  Let \tcode{T1} denote the second type in \tcode{Ts}.
  Let \tcode{TRest} be a pack that denotes \tcode{Ts...} without the first two types.
  ...
  \begin{itemize}
    ...
    \item \tcode{common_type_t<\mathcommonsimd<T0>, \mathcommonsimd<T1>>}, if \tcode{sizeof...(Ts)}
      equals 2; otherwise

    \item \tcode{common_type_t<\mathcommonsimd<T0, T1>, TRest...>}, if
      \tcode{common_type<\mathcommonsimd<T0>, \mathcommonsimd<T1>>} has a type member \tcode{type};
      otherwise
...
\tcode{basic_simd} result, where
\INFO{Better?\\
Unless otherwise specified, each mathematical function declared in \tcode{<simd>} returns a prvalue
of a data-parallel type, where [\ldots]
}\noindent
the value of each element approximates the result obtained by
...
\begin{itemdescr}
  ...
  \INFO{except \tcode{errno} is not modified}
\end{itemdescr}
