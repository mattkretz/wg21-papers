...
namespace std@\wgRem{::experimental \{ inline namespace parallelism_v2}@ {
  namespace simd_abi {
    ...
    @\wgAdd{template<int N> using abi_stable = \seebelow;}@
  ...
  @\wgAdd{template<class T, int N> using abi_stable_simd = simd<T, simd_abi::abi_stable<N>>;}@
  ...
  @\wgAdd{template<class T, int N> using abi_stable_simd_mask = simd_mask<T, simd_abi::abi_stable<N>>;}@
...
\pnum
\tcode{scalar} is an alias for an unspecified ABI tag that is different from \tcode{fixed_size<1>}\wgAdd{ and \tcode{abi_stable<1>}}.
...
elements (i.e. \tcode{\wgChange{simd<T, simd_abi::fixed_size<N>>::size()}{simd_size_v<T,
simd_abi::fixed_size<N>>}} is \tcode{N}). \tcode{simd<T, fixed_size<N>>} and \tcode{simd_mask<T,
...
\pnum
\wgAdd{%
\tcode{abi_stable<N>} is an alias for an unspecified ABI tag.
\tcode{abi_stable} does not introduce a non-deduced context.
Use of the \tcode{simd_abi::abi_stable<N>} tag type requires data-parallel types to store \tcode{N}
elements (i.e. \tcode{simd_size_v<T, simd_abi::abi_stable<N>>} is \tcode{N}).
\tcode{abi_stable_simd<T, N>} and \tcode{abi_stable_simd_mask<T, N>} are supported iff
\tcode{fixed_size_simd<T, N>} and \tcode{fixed_size_simd_mask<T, N>} are supported.
}
...
\wgAdd{%
  ...
  The type of \tcode{abi_stable<N>} is the same in TU1 and TU2.
}
\end{wgBAdd}

\pnum
\begin{note}
  An implementation \wgChange{can forego}{should ensure} ABI compatibility between differently compiled translation
  units for \tcode{simd} and \tcode{simd_mask} specializations using the same
  \tcode{simd_abi::\wgChange{fixed_size}{abi_stable}<N>} tag.\wgRem{ Otherwise, the efficiency of
...
\begin{note}
  ...
  The \tcode{value} member is present even if \tcode{simd<T, Abi>} is not supported for the currently targeted system.}
\end{note}
...
\begin{itemdescr}
  \pnum\returns
  A \tcode{simd_mask} object with the $i^\text{th}$ element initialized to \tcode{x[$i$]} \foralli.

  \pnum\remarks
  The functions shall not participate in overload resolution unless either
  \begin{itemize}
    \item \tcode{is_simd_mask_v<T>} is \tcode{false}, or
    \item \tcode{T::size() == simd_size_v<U, Abi>} is \tcode{true}.
  \end{itemize}

  \pnum
  The return type is
  \begin{itemize}
    \item \tcode{T} if \tcode{is_simd_mask_v<T>} is \tcode{true};
    \item otherwise, \tcode{simd_mask<T, Abi>} if either \tcode{U} is the same type as \tcode{T} or \tcode{make_signed_t<U>} is the same type as \tcode{make_signed_t<T>};
    \item otherwise, \tcode{simd_mask<T, simd_abi::fixed_size<simd_size_v<U, Abi>>>}
  \end{itemize}
\end{itemdescr}
\end{wgBAdd}
