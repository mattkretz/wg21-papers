...
\pnum
The simd subclause defines data-parallel types and operations on these types.
\begin{note}
...
([basic.fundamental]).
In addition,
if \tcode{__STDCPP_FLOAT16_T__} is defined, \tcode{std::float16_t} is a vectorizable type;
if \tcode{__STDCPP_FLOAT32_T__} is defined, \tcode{std::float32_t} is a vectorizable type;
if \tcode{__STDCPP_FLOAT64_T__} is defined, \tcode{std::float64_t} is a vectorizable type
([basic.extended.fp]).
...
namespace std {
  ...
  template<size_t Bs, class... Abis>
    constexpr constexpr basic_simd_mask<Bs, @\deducet@<@\integerfrom@<Bs>,
...
of \tcode{\deducet<T, N>} in translation unit 2 iff the type of
