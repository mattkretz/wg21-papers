...
\begin{note}
  \tcode{basic_simd<T>} is intended to have the natural width suggested by the architecture of the
  execution environment.
  The other \tcode{basic_simd<T, Abi>} specializations are intended to be enabled if the
  architecture of the execution environment allows.
\end{note}
\FIXME{LWG decided: preference to keep the note. Instead of “currently targeted system”, can we
talk about “expected execution environment”?}
...
\begin{itemdescr}
  ...
  \tcode{\placeholdernc{GENERALIZED_SUM}(binary_op, simd<T, 1>(x[$k_0$]), $\ldots$, simd<T,
  1>(x[$k_n$]))[0]} where $k_0, \ldots, k_n$ are the selected indices of \tcode{mask}.
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\returns
  If \tcode{none_of(mask)} is \tcode{true}, returns \tcode{T(1)}. Otherwise, returns
  \tcode{\placeholdernc{GENERALIZED_SUM}(binary_op, simd<T, 1>(x[$k_0$]), $\ldots$, simd<T,
  1>(x[$k_n$]))[0]} where $k_0, \ldots, k_n$ are the selected indices of \tcode{mask}.
\end{itemdescr}
...
\begin{note}
  \tcode{basic_simd_mask<Bytes>} is intended to have the natural width suggested by the architecture
  of the execution environment.
  The other \tcode{basic_simd_mask<Bytes, Abi>} specializations are intended to be enabled if the
  architecture of the execution environment allows.
\end{note}
