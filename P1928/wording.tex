...
namespace std {
  ...
  // \ref{simd.creation}, \tcode{simd} and \tcode{simd_mask} creation
...
\rSec2[simd.flags.overview]{Class template \tcode{loadstore_flags} overview}

\begin{codeblock}
template <class... Flags> struct loadstore_flags {
  // \ref{simd.flags.oper}, \tcode{loadstore_flags} operators
...
\rSec2[simd.flags.oper]{\tcode{loadstore_flags} operators}
...
\begin{itemdescr}
  \pnum\constraints
  \tcode{simd(gen(integral_constant<size_t, i>()))} is well-formed \foralli.
...
\rSec2[simd.cond]{\tcode{simd} conditional operators}
...
\end{itemdescr}
\rSec2[simd.reductions]{\tcode{simd} reductions}
...
\rSec2[simd.creation]{\tcode{simd} and \tcode{simd_mask} creation}
...
\rSec2[simd.math]{\tcode{simd} math library}
...
public:
  ...
  template <class T0, class T1>
    friend constexpr simd<@\seebelow@, Abi>
...
\rSec2[simd.mask.ctor]{\tcode{simd_mask} constructors}
...
\rSec2[simd.mask.copy]{\tcode{simd_mask} copy functions}
...
\rSec2[simd.mask.subscr]{\tcode{simd_mask} subscript operators}
...
\rSec2[simd.mask.unary]{\tcode{simd_mask} unary operators}
...
\rSec2[simd.mask.conv]{\tcode{simd_mask} conversion operators}
...
\rSec2[simd.mask.binary]{\tcode{simd_mask} binary operators}
...
\rSec2[simd.mask.cassign]{\tcode{simd_mask} compound assignment}
...
\rSec2[simd.mask.comparison]{\tcode{simd_mask} comparisons}
...
\rSec2[simd.mask.cond]{\tcode{simd_mask} conditional operators}
...
\begin{itemdescr}
  ...
  \pnum\returns
  A \tcode{simd<U, Abi>} object where the $i^\text{th}$ element equals \tcode{mask[$i$] ? a : b} \foralli.
\end{itemdescr}

\rSec2[simd.mask.reductions]{\tcode{simd_mask} reductions}
