...
\pnum
Throughout this Clause, the term \defn{data-parallel type} refers to all \defn{supported} (\ref{simd.overview}) specializations of the \tcode{simd} and \tcode{simd_mask} class templates. A \defn{data-parallel object} is an object of \term{data-parallel type}.
...
\rSec1[simd.synopsis]{Header \texorpdfstring{\tcode{<\wgRem{experimental/}simd>}}{<simd>} synopsis}

%\indexhdr{simd}
\begin{codeblock}
namespace std@\wgRem{::experimental \{ inline namespace parallelism_v2}@ {
  namespace simd_abi {
    using scalar = @\seebelow@;
    template<@\wgAdd{class T, }\wgChange{int}{size_t}@ N> using fixed_size = @\seebelow@;
    template<class T> inline constexpr @\wgChange{int}{size_t}@ max_fixed_size = @\impdef;\diffbreak{4}%
    \wgRem{template<class T> using compatible = \impdef;}@
  ...
  template<class T, class Abi = simd_abi::@\wgChange{compatible}{native}@<T>> struct simd_size;
  template<class T, class Abi = simd_abi::@\wgChange{compatible}{native}@<T>>
  ...
  template<@\wgChange{int}{size_t}@ N, class V> struct resize_simd { using type = @\seebelow@; };
  template<@\wgChange{int}{size_t}@ N, class V> using resize_simd_t = typename resize_simd<N, V>::type;

  // \ref{simd.class}, Class template \tcode{simd}
  template<class T, class Abi = simd_abi::@\wgChange{compatible}{native}@<T>> class simd;@\diffbreak{2}
  \wgRem{template<class T> using native_simd = simd<T, simd_abi::native<T>>;}@
  template<class T, @\wgChange{int}{size_t}@ N> using fixed_size_simd = simd<T, simd_abi::fixed_size<@\wgAdd{T, }@N>>;

  // \ref{simd.mask.class}, Class template \tcode{simd_mask}
  template<class T, class Abi = simd_abi::@\wgChange{compatible}{native}@<T>> class simd_mask;@\diffbreak{2}
  \wgRem{template<class T> using native_simd_mask = simd_mask<T, simd_abi::native<T>>;}@
  template<class T, @\wgChange{int}{size_t}@ N> using fixed_size_simd_mask = simd_mask<T, simd_abi::fixed_size<@\wgAdd{T, }@N>>;

  @\wgRem{// \mbox{\ref{simd.casts}}, Casts}\diffbreak{2}%
  \wgRem{template<class T, class U, class Abi> \seebelow{} simd_cast(const simd<U, Abi>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, class U, class Abi> \seebelow{} static_simd_cast(const simd<U, Abi>\&) noexcept;}\diffbreak{0}%
  \diffbreak{2}%
  \wgRem{template<class T, class Abi>}\diffbreak{4}%
    \wgRem{fixed_size_simd<T, simd_size_v<T, Abi>>}\diffbreak{6}%
    \wgRem{to_fixed_size(const simd<T, Abi>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, class Abi>}\diffbreak{4}%
    \wgRem{fixed_size_simd_mask<T, simd_size_v<T, Abi>>}\diffbreak{6}%
    \wgRem{to_fixed_size(const simd_mask<T, Abi>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, int N>}\diffbreak{4}%
    \wgRem{native_simd<T> to_native(const fixed_size_simd<T, N>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, int N>}\diffbreak{4}%
    \wgRem{native_simd_mask<T> to_native(const fixed_size_simd_mask<T, N>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, int N>}\diffbreak{4}%
    \wgRem{simd<T> to_compatible(const fixed_size_simd<T, N>\&) noexcept;}\diffbreak{2}%
  \wgRem{template<class T, int N>}\diffbreak{4}%
    \wgRem{simd_mask<T> to_compatible(const fixed_size_simd_mask<T, N>\&) noexcept;}\diffbreak{0}%
@
  template<size_t... Sizes, class T, class Abi>
    @\wgAdd{constexpr }@tuple<simd<T, simd_abi::deduce_t<T, Sizes>>...>
      split(const simd<T, Abi>&) noexcept;
  template<size_t... Sizes, class T, class Abi>
    @\wgAdd{constexpr }@tuple<simd_mask<T, simd_mask_abi::deduce_t<T, Sizes>>...>
      split(const simd_mask<T, Abi>&) noexcept;
  template<class V, class Abi>
    @\wgAdd{constexpr }@array<V, simd_size_v<typename V::value_type, Abi> / V::size()>
      split(const simd<typename V::value_type, Abi>&) noexcept;
  template<class V, class Abi>
    @\wgAdd{constexpr }@array<V, simd_size_v<typename V::simd_type::value_type, Abi> / V::size()>
      split(const simd_mask<typename V::simd_type::value_type, Abi>&) noexcept;

  template<size_t N, class T, class A>
    @\wgAdd{constexpr }@array<resize_simd@\wgAdd{_t}@<simd_size_v<T, A> / N, simd<T, A>>, N>
      split_by(const simd<T, A>& x) noexcept;
  template<size_t N, class T, class A>
    @\wgAdd{constexpr }@array<resize_simd@\wgAdd{_t}@<simd_size_v<T, A> / N, simd_mask<T, A>>, N>
      split_by(const simd_mask<T, A>& x) noexcept;

  template<class T, class... Abis>
    @\wgAdd{constexpr }@simd<T, simd_abi::deduce_t<T, (simd_size_v<T, Abis> + ...)>>
      concat(const simd<T, Abis>&...) noexcept;
  template<class T, class... Abis>
    @\wgAdd{constexpr }@simd_mask<T, simd_abi::deduce_t<T, (simd_size_v<T, Abis> + ...)>>
      concat(const simd_mask<T, Abis>&...) noexcept;

  template<class T, class Abi, size_t N>
    @\wgAdd{constexpr }@resize_simd@\wgAdd{_t}@<simd_size_v<T, Abi> * N, simd<T, Abi>>
      concat(const array<simd<T, Abi>, N>& arr) noexcept;
  template<class T, class Abi, size_t N>
    @\wgAdd{constexpr }@resize_simd@\wgAdd{_t}@<simd_size_v<T, Abi> * N, simd_mask<T, Abi>>
      concat(const array<simd_mask<T, Abi>, N>& arr) noexcept;

  // \ref{simd.mask.reductions}, \wgChange{R}{\tcode{simd_mask} r}eductions
  template<class T, class Abi> @\wgAdd{constexpr }@bool all_of(const simd_mask<T, Abi>&) noexcept;
  template<class T, class Abi> @\wgAdd{constexpr }@bool any_of(const simd_mask<T, Abi>&) noexcept;
  template<class T, class Abi> @\wgAdd{constexpr }@bool none_of(const simd_mask<T, Abi>&) noexcept;
  template<class T, class Abi> @\wgAdd{constexpr }@bool some_of(const simd_mask<T, Abi>&) noexcept;
  template<class T, class Abi> @\wgAdd{constexpr }@int popcount(const simd_mask<T, Abi>&) noexcept;
  template<class T, class Abi> @\wgAdd{constexpr }@int find_first_set(const simd_mask<T, Abi>&);
  template<class T, class Abi> @\wgAdd{constexpr }@int find_last_set(const simd_mask<T, Abi>&);

  @\wgAdd{constexpr }@bool all_of(@\UNSP{T}@) noexcept;
  @\wgAdd{constexpr }@bool any_of(@\UNSP{T}@) noexcept;
  @\wgAdd{constexpr }@bool none_of(@\UNSP{T}@) noexcept;
  @\wgAdd{constexpr }@bool some_of(@\UNSP{T}@) noexcept;
  @\wgAdd{constexpr }@int popcount(@\UNSP{T}@) noexcept;
  @\wgAdd{constexpr }@int find_first_set(@\UNSP{T}@);
  @\wgAdd{constexpr }@int find_last_set(@\UNSP{T}@);
  ...
  // \ref{simd.reductions}, \wgChange{R}{\tcode{simd} r}eductions
  template<class T, class Abi, class BinaryOperation = plus<>>
    @\wgAdd{constexpr }@T reduce(const simd<T, Abi>&,
                       BinaryOperation = {});

  template<class M, class V, class BinaryOperation>
    @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x,
  ...
  template<class T, class Abi>
    @\wgAdd{constexpr }@T hmin(const simd<T, abi>&) noexcept;
  template<class M, class V>
    @\wgAdd{constexpr }@typename V::value_type hmin(const const_where_expression<M, V>&) noexcept;
  template<class T, class Abi>
    @\wgAdd{constexpr }@T hmax(const simd<T, abi>&) noexcept;
  template<class M, class V>
    @\wgAdd{constexpr }@typename V::value_type hmax(const const_where_expression<M, V>&) noexcept;

  // \ref{simd.alg}, Algorithms
  template<class T, class Abi>
    @\wgAdd{constexpr }@simd<T, Abi>
      min(const simd<T, Abi>& a, const simd<T, Abi>& b) noexcept;
  template<class T, class Abi>
    @\wgAdd{constexpr }@simd<T, Abi>
      max(const simd<T, Abi>& a, const simd<T, Abi>& b) noexcept;
  template<class T, class Abi>
    @\wgAdd{constexpr }@pair<simd<T, Abi>, simd<T, Abi>>
...
\pnum
The header \tcode{<\wgRem{experimental/}simd>} defines class templates, tag types, trait types, and function templates for element-wise operations on data-parallel objects.
...
namespace simd_abi {
  using scalar = @\seebelow@;
  template<@\wgAdd{class T, }@@\wgChange{int}{size_t}@ N> using fixed_size = @\seebelow@;
  template<class T> inline constexpr @\wgChange{int}{size_t}@ max_fixed_size = @\impdef@;@\diffbreak{2}
  \wgRem{template<class T> using compatible = \impdef;}@
  template<class T> using native = @\impdef@;
}
\end{codeblock}
...
Use of the \tcode{scalar} tag type requires data-parallel types to store a single element (i.e., \wgChange{\tcode{simd<T, simd_abi::scalar>::size()} returns}{\tcode{simd_size_v<T, simd_abi::scalar>} equals} 1).
...
elements (i.e.
\wgChange
  {\tcode{simd<T, simd_abi::fixed_size<N>>::size()\relax} is}
  {\tcode{simd_size_v<T, simd_abi::fixed_size<N>>\relax} equals}
...
\wgRem{
\noteintro{Note}
  An implementation can forego ABI compatibility between differently compiled translation
  units for \tcode{simd} and \tcode{simd_mask} specializations using the same
  \tcode{simd_abi::fixed_size<N>} tag. Otherwise, the efficiency of
  \tcode{simd<T, Abi>} is likely to be better than for \tcode{simd<T,
  fixed_size<simd_size_v<T, Abi>>>} (with \tcode{Abi} not a specialization
  of \tcode{simd_abi::fixed_size}).
\noteoutro{note}
}
\end{wgBRem}

\pnum
An implementation may define additional \defn{extended ABI tag} types in the \tcode{std::\wgRem{experimental::parallelism_v2::}simd_abi} namespace, to support other forms of data-parallel computation.

\begin{wgBRem}
\pnum
\wgRem{\tcode{compatible<T>} is an implementation-defined alias for an ABI tag.
\noteintro{Note}
The intent is to use the ABI tag producing the most efficient data-parallel execution for the element type \tcode{T} that ensures ABI compatibility between translation units on the target architecture.
\noteoutro{note}}
\begin{example}
  \wgRem{Consider a target architecture supporting the extended ABI tags \tcode{__simd128} and \tcode{__simd256}, where the \tcode{__simd256} type requires an optional ISA extension on said architecture. Also, the target architecture does not support \tcode{long double} with either ABI tag. The implementation therefore defines \tcode{compatible<T>} as an alias for:}
  \begin{itemize}
    \wgItemRem\wgRem{\tcode{scalar} if \tcode{T} is the same type as \tcode{long double}, and}
    \wgItemRem\wgRem{\tcode{__simd128} otherwise.}
  \end{itemize}
\end{example}
\end{wgBRem}

\pnum
\tcode{native<T>} is an implementation-defined alias for an ABI tag. \begin{note}The intent is to use the ABI tag producing the most efficient data-parallel execution for the element type \tcode{T} that is supported on the currently targeted system. \wgRem{For target architectures without ISA extensions, the \tcode{native<T>} and \tcode{compatible<T>} aliases will likely be the same.} For target architectures with ISA extensions, compiler flags may \wgChange{influence}{change the type of} the \tcode{native<T>} alias\wgRem{while \tcode{compatible<T>} will be the same independent of such flags}.\end{note}
\begin{example}
...
\wgRem{Otherwise, if there are multiple ABI tag types that satisfy the constraints, the member typedef \tcode{type} is implementation-defined.}
\begin{note}\wgRem{It is expected that extended ABI tags can produce better optimizations and thus are preferred over \tcode{simd_abi::fixed_size<N>}.}
...
\begin{itemdecl}
template<class T, class Abi = simd_abi::@\wgChange{compatible}{native}@<T>> struct simd_size { @\seebelow@ };
\end{itemdecl}
...
\end{itemize}
\begin{note}
  The rules are different from those in (\ref{simd.overview})\wgChange{.}{:
  The member \tcode{value} is present even if \tcode{simd<T, Abi>} is not supported for the currently targeted system.}
\end{note}

\pnum
If \tcode{value} is present, the type \tcode{simd_size<T, Abi>} is a \tcode{BinaryTypeTrait} with a base characteristic of \tcode{integral_constant<size_t, N>} with \tcode{N} equal to the number of elements in a \tcode{simd<T, Abi>} object. \wgRem{\noteintro{Note}If \tcode{simd<T, Abi>} is not supported for the currently targeted system, \tcode{simd_size<T, Abi>::value} produces the value \tcode{simd<T, Abi>::size()} would return if it were supported.\noteoutro{note}}
...
\rSec1[simd.whereexpr]{Where expression class templates}
...
public:
  ...
  template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> void copy_to(U* mem, Flags f@\wgAdd{ = \{\}}@) const &&;
};
...
public:
  ...
  template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> void copy_from(const U* mem, Flags@\wgAdd{ = \{\}}@) &&;
};
\end{codeblock}
...
\pnum
\begin{note}
  The \tcode{where} functions \ref{simd.mask.where} initialize \tcode{mask} with the first argument to \tcode{where} and \tcode{data} with the second argument to \tcode{where}.
\end{note}
...
\begin{itemdecl}
template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> void copy_to(U* mem, Flags@\wgAdd{ = \{\}}@) const &&;
\end{itemdecl}
...
public:
  ...
  static constexpr @\wgChange{size_t size() noexcept}{typename simd_size<T, Abi>::type size}@;

  @\wgAdd{constexpr }@simd() noexcept = default;

  // \ref{simd.ctor}, \tcode{simd} constructors
  template<class U> @\wgAdd{constexpr }@simd(U&& value) noexcept;
  template<class U@\wgAdd{, class UAbi}@> @\diffbreak{4}
    \wgAdd{constexpr explicit(\seebelow)}@ simd(const simd<U, @\wgChange{simd_abi::fixed_size<size()>}{UAbi}@>&) noexcept;
  template<class G> @\wgAdd{constexpr }@explicit simd(G&& gen) noexcept;
  template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@simd(const U* mem, Flags f@\wgAdd{ = \{\}}@);

  // \ref{simd.copy}, \tcode{simd} copy functions
  template<class U, class Flags@\wgAdd{ = element_aligned_tag}@>
    @\wgAdd{constexpr void}@ copy_from(const U* mem, Flags f@\wgAdd{ = \{\}}@);
  template<class U, class Flags@\wgAdd{ = element_aligned_tag}@>
    @\wgAdd{constexpr void}@ copy_to(U* mem, Flags f@\wgAdd{ = \{\}}@)@\wgAdd{ const}@;

  // \ref{simd.subscr}, \tcode{simd} subscript operators
  @\wgAdd{constexpr }@reference operator[](size_t);
  @\wgAdd{constexpr }@value_type operator[](size_t) const;

  // \ref{simd.unary}, \tcode{simd} unary operators
  @\wgAdd{constexpr }@simd& operator++() noexcept;
  @\wgAdd{constexpr }@simd operator++(int) noexcept;
  @\wgAdd{constexpr }@simd& operator--() noexcept;
  @\wgAdd{constexpr }@simd operator--(int) noexcept;
  @\wgAdd{constexpr }@mask_type operator!() const noexcept;
  @\wgAdd{constexpr }@simd operator~() const noexcept;
  @\wgAdd{constexpr }@simd operator+() const noexcept;
  @\wgAdd{constexpr }@simd operator-() const noexcept;

  // \ref{simd.binary}, \tcode{simd} binary operators
  friend @\wgAdd{constexpr }@simd operator+(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator-(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator*(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator/(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator%(const simd&, const simd&) noexcept;
  ...
  friend @\wgAdd{constexpr }@simd operator<<(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator>>(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd operator<<(const simd&, int) noexcept;
  friend @\wgAdd{constexpr }@simd operator>>(const simd&, int) noexcept;

  // \ref{simd.cassign}, \tcode{simd} compound assignment
  friend @\wgAdd{constexpr }@simd& operator+=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator-=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator*=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator/=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator%=(simd&, const simd&) noexcept;
  ...
  friend @\wgAdd{constexpr }@simd& operator<<=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator>>=(simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@simd& operator<<=(simd&, int) noexcept;
  friend @\wgAdd{constexpr }@simd& operator>>=(simd&, int) noexcept;

  // \ref{simd.comparison}, \tcode{simd} compare operators
  friend @\wgAdd{constexpr }@mask_type operator==(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@mask_type operator!=(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@mask_type operator>=(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@mask_type operator<=(const simd&, const simd&) noexcept;
  friend @\wgAdd{constexpr }@mask_type operator>(const simd&, const simd&) noexcept;
...
\pnum
Every specialization of \tcode{simd} \wgChange{shall be}{is} a complete type. The specialization \tcode{simd<T, Abi>} is supported if \tcode{T} is a vectorizable type and
\begin{itemize}
...
\begin{codeblock}
@\wgAdd{constexpr }@explicit operator @\impdef@() const;
@\wgAdd{constexpr }@explicit simd(const @\impdef@& init);
\end{codeblock}
...
public:
  ...
  @\wgAdd{constexpr }@operator value_type() const noexcept;

  template<class U> @\wgAdd{constexpr }@reference operator=(U&& x) && noexcept;
  ...
  @\wgAdd{constexpr }@reference operator++() && noexcept;
  @\wgAdd{constexpr }@value_type operator++(int) && noexcept;
  @\wgAdd{constexpr }@reference operator--() && noexcept;
  @\wgAdd{constexpr }@value_type operator--(int) && noexcept;

  friend @\wgAdd{constexpr }@void swap(reference&& a, reference&& b) noexcept;
  friend @\wgAdd{constexpr }@void swap(value_type& a, reference&& b) noexcept;
  friend @\wgAdd{constexpr }@void swap(reference&& a, value_type& b) noexcept;
};
\end{codeblock}
...
\begin{itemdecl}
template<class U> @\wgAdd{constexpr }@simd(U&&) noexcept;
\end{itemdecl}
...
\begin{itemdescr}
  ...
  \pnum\remarks
  Let \tcode{From} denote the type \wgChange{\tcode{remove_cv_t<remove_reference_t<U>>}}{\tcode{remove_cvref_t<U>}}. This constructor shall not participate in overload resolution unless:
  \begin{itemize}
    ...
    \item \tcode{From} is \tcode{unsigned int} and \wgChange{\tcode{value_type} is an unsigned integral type}{\tcode{is_unsigned_v<value_type>} is \tcode{true}}.% unsigned integer types excludes character types
  \end{itemize}

\end{itemdescr}

\begin{itemdecl}
template<class U@\wgAdd{, class UAbi}@> @\diffbreak{2}\wgAdd{constexpr explicit(\seebelow) }@simd(const simd<U, @\wgChange{simd_abi::fixed_size<size()>}{UAbi}@>& x) noexcept;
\end{itemdecl}
...
\begin{itemdescr}
  ...
  \wgAdd{\tcode{simd_size_v<U, UAbi> == size()}.}
  \begin{itemize}
    \wgItemRem \wgRem{\tcode{abi_type} is \tcode{simd_abi::fixed_size<size()>}, and}
    \wgItemRem \wgRem{every possible value of \tcode{U} can be represented with type \tcode{value_type}, and}
    \wgItemRem \wgRem{if both \tcode{U} and \tcode{value_type} are integral, the integer conversion rank (\mbox{\ref{conv.rank}}) of \tcode{value_type} is greater than the integer conversion rank of \tcode{U}.}
  \end{itemize}

  \begin{wgBAdd}
    \pnum
    \wgAdd{The constructor is \tcode{explicit} iff}
    \begin{itemize}
      \wgItemAdd \wgAdd{at least one possible value of \tcode{U} cannot be represented with type \tcode{value_type}, or}
      \wgItemAdd \wgAdd{if both \tcode{U} and \tcode{value_type} are integral types, the integer conversion rank (\mbox{\ref{conv.rank}}) of \tcode{U} is greater than the integer conversion rank of \tcode{value_type}, or}
      \wgItemAdd \wgAdd{if both \tcode{U} and \tcode{value_type} are floating-point types, the floating-point conversion rank (\mbox{\ref{conv.rank}}) of \tcode{U} is greater than the floating-point conversion rank of \tcode{value_type}.}
    \end{itemize}
  \end{wgBAdd}
\end{itemdescr}

\begin{itemdecl}
template<class G> @\wgAdd{constexpr }@simd(G&& gen) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@simd(const U* mem, Flags@\wgAdd{ = \{\}}@);
\end{itemdecl}
...
\begin{itemdecl}
template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@void copy_from(const U* mem, Flags@\wgAdd{ = \{\}}@);
\end{itemdecl}
...
\begin{itemdecl}
template<class U, class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@void copy_to(U* mem, Flags@\wgAdd{ = \{\}}@) const;
\end{itemdecl}
...
\begin{itemdecl}
@\wgAdd{constexpr }@reference operator[](size_t i);
\end{itemdecl}
...
\begin{itemdecl}
@\wgAdd{constexpr }@value_type operator[](size_t i) const;
\end{itemdecl}
...
\begin{itemdecl}
friend @\wgAdd{constexpr }@simd operator+(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd operator-(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd operator*(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd operator/(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd operator%(const simd& lhs, const simd& rhs) noexcept;
...
\begin{itemdecl}
friend @\wgAdd{constexpr }@simd operator<<(const simd& v, int n) noexcept;
friend @\wgAdd{constexpr }@simd operator>>(const simd& v, int n) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
friend @\wgAdd{constexpr }@simd& operator+=(simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd& operator-=(simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd& operator*=(simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd& operator/=(simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@simd& operator%=(simd& lhs, const simd& rhs) noexcept;
...
\begin{itemdecl}
friend @\wgAdd{constexpr }@simd& operator<<=(simd& lhs, int n) noexcept;
friend @\wgAdd{constexpr }@simd& operator>>=(simd& lhs, int n) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
friend @\wgAdd{constexpr }@mask_type operator==(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@mask_type operator!=(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@mask_type operator>=(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@mask_type operator<=(const simd& lhs, const simd& rhs) noexcept;
friend @\wgAdd{constexpr }@mask_type operator>(const simd& lhs, const simd& rhs) noexcept;
...
\rSec2[simd.reductions]{\texorpdfstring{\wgChange{R}{\tcode{simd} r}}{simd r}eductions}
...
\begin{itemdecl}
template<class T, class Abi, class BinaryOperation = plus<>>
  @\wgAdd{constexpr }@T reduce(const simd<T, Abi>& x, BinaryOperation binary_op = {});
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V>
  @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x, plus<> binary_op) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V>
  @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x, multiplies<> binary_op) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V>
  @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x, bit_and<> binary_op) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V>
  @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x, bit_or<> binary_op) noexcept;
template<class M, class V>
  @\wgAdd{constexpr }@typename V::value_type reduce(const const_where_expression<M, V>& x, bit_xor<> binary_op) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@T hmin(const simd<T, Abi>& x) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V> @\wgAdd{constexpr }@typename V::value_type hmin(const const_where_expression<M, V>& x) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@T hmax(const simd<T, Abi>& x) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class M, class V> @\wgAdd{constexpr }@typename V::value_type hmax(const const_where_expression<M, V>& x) noexcept;
\end{itemdecl}
...
\begin{wgBRem}
\begin{itemdecl}
@\wgRem{template<class T, class U, class Abi> \seebelow simd_cast(const simd<U, Abi>\& x) noexcept;}@
\end{itemdecl}

\begin{itemdescr}
  \pnum
  \wgRem{Let \tcode{To} denote \tcode{T::value_type} if \tcode{is_simd_v<T>} is \tcode{true}, or \tcode{T} otherwise.}
  ...
  \begin{itemize}
    \wgItemRem \wgRem{every possible value of type \tcode{U} can be represented with type \tcode{To}, and}
    \wgItemRem \wgRem{either}
      \begin{itemize}
        \wgItemRem \wgRem{\tcode{is_simd_v<T>} is \tcode{false}, or}
        \wgItemRem \wgRem{\tcode{T::size() == simd<U, Abi>::size()} is \tcode{true}.}
      \end{itemize}
  \end{itemize}

  \pnum
  \wgRem{The return type is}
  \begin{itemize}
    \wgItemRem \wgRem{\tcode{T} if \tcode{is_simd_v<T>} is \tcode{true};}
    \wgItemRem \wgRem{otherwise, \tcode{simd<T, Abi>} if \tcode{U} is the same type as \tcode{T};}
    \wgItemRem \wgRem{otherwise, \tcode{simd<T, simd_abi::fixed_size<simd<U, Abi>::size()>>}}
  \end{itemize}
\end{itemdescr}

\begin{itemdecl}
@\wgRem{template<class T, class U, class Abi> \seebelow static_simd_cast(const simd<U, Abi>\& x) noexcept;}@
\end{itemdecl}
...
\begin{itemdescr}
  ...
  \begin{itemize}
    \wgItemRem \wgRem{\tcode{T} if \tcode{is_simd_v<T>} is \tcode{true};}
    \wgItemRem \wgRem{otherwise, \tcode{simd<T, Abi>} if either \tcode{U} is the same type as \tcode{T} or \tcode{make_signed_t<U>} is the same type as \tcode{make_signed_t<T>};}
    \wgItemRem \wgRem{otherwise, \tcode{simd<T, simd_abi::fixed_size<simd<U, Abi>::size()>>}.}
  \end{itemize}
\end{itemdescr}

\begin{itemdecl}
@\wgRem{template<class T, class Abi>}\diffbreak{2}%
  \wgRem{fixed_size_simd<T, simd_size_v<T, Abi>> to_fixed_size(const simd<T, Abi>\& x) noexcept;}\diffbreak{0}%
\wgRem{template<class T, class Abi>}\diffbreak{2}%
  \wgRem{fixed_size_simd_mask<T, simd_size_v<T, Abi>> to_fixed_size(const simd_mask<T, Abi>\& x) noexcept;}@
\end{itemdecl}
...
\begin{itemdecl}
@\wgRem{template<class T, int N> native_simd<T> to_native(const fixed_size_simd<T, N>\& x) noexcept;}\diffbreak{0}%
\wgRem{template<class T, int N> native_simd_mask<T> to_native(const fixed_size_simd_mask<T, N>\& x) noexcept;}@
\end{itemdecl}
...
\begin{itemdecl}
@\wgRem{template<class T, int N> simd<T> to_compatible(const fixed_size_simd<T, N>\& x) noexcept;}\diffbreak{0}%
\wgRem{template<class T, int N> simd_mask<T> to_compatible(const fixed_size_simd_mask<T, N>\& x) noexcept;}@
\end{itemdecl}
...
template<size_t... Sizes, class T, class Abi>
  @\wgAdd{constexpr }@tuple<simd_mask<T, simd_abi::deduce_t<T, Sizes>>...>
...
\begin{itemdecl}
template<size_t N, class T, class A>
  @\wgAdd{constexpr }@array<resize_simd<simd_size_v<T, A> / N, simd<T, A>>, N>
    split_by(const simd<T, A>& x) noexcept;
template<size_t N, class T, class A>
  @\wgAdd{constexpr }@array<resize_simd<simd_size_v<T, A> / N, simd_mask<T, A>>, N>
    split_by(const simd_mask<T, A>& x) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class... Abis>
  @\wgAdd{constexpr }@simd<T, simd_abi::deduce_t<T, (simd_size_v<T, Abis> + ...)>> concat(
    const simd<T, Abis>&... xs) noexcept;
template<class T, class... Abis>
  @\wgAdd{constexpr }@simd_mask<T, simd_abi::deduce_t<T, (simd_size_v<T, Abis> + ...)>> concat(
...
\begin{itemdecl}
  template<class T, class Abi, size_t N>
    @\wgAdd{constexpr }@resize_simd<simd_size_v<T, Abi> * N, simd<T, Abi>>
      concat(const array<simd<T, Abi>, N>& arr) noexcept;
  template<class T, class Abi, size_t N>
    @\wgAdd{constexpr }@resize_simd<simd_size_v<T, Abi> * N, simd_mask<T, Abi>>
      concat(const array<simd_mask<T, Abi>, N>& arr) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@simd<T, Abi> min(const simd<T, Abi>& a, const simd<T, Abi>& b) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@simd<T, Abi> max(const simd<T, Abi>& a, const simd<T, Abi>& b) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi>
  @\wgAdd{constexpr }@pair<simd<T, Abi>, simd<T, Abi>> minmax(const simd<T, Abi>& a, const simd<T, Abi>& b) noexcept;
\end{itemdecl}
...
template<class T, class Abi> simd<T, Abi>
  @\wgAdd{constexpr }@clamp(const simd<T, Abi>& v, const simd<T, Abi>& lo, const simd<T, Abi>& hi);
\end{itemdecl}
...
\rSec2[simd.math]{\texorpdfstring{\wgChange{M}{\tcode{simd} m}ath}{simd math} library}
...
\pnum
The \wgChange{behavior is undefined}{result is unspecified} if a domain, pole, or range error occurs when the input argument(s) are applied to the indicated \tcode{<cmath>} function.
\wgAdd{\noteintro{Note} Implementations are encouraged to follow the C specification (especially Annex F). \noteoutro{note}}
...
public:
  ...
  @\wgAdd{constexpr }@simd_mask() noexcept = default;

  // \ref{simd.mask.ctor}, \wgChange{C}{\tcode{simd_mask} c}onstructors
  @\wgAdd{constexpr} @explicit simd_mask(value_type) noexcept;
  template<class U@\wgAdd{, class UAbi}@>
    @\wgAdd{constexpr explicit(sizeof(U) != sizeof(T)) }@simd_mask(const simd_mask<U, @\wgChange{simd_abi::fixed_size<size()>}{UAbi}@>&) noexcept;
  @\wgAdd{template<class G> constexpr explicit simd_mask(G\&\& gen) noexcept;}@
  template<class Flags@\wgAdd{ = element_aligned_tag}@>
    @\wgAdd{constexpr }@simd_mask(const value_type* mem, Flags@\wgAdd{ = \{\}}@);

  // \ref{simd.mask.copy}, \wgChange{C}{\tcode{simd_mask} c}opy functions
  template<class Flags@\wgAdd{ = element_aligned_tag}@>
    @\wgAdd{constexpr}@ void copy_from(const value_type* mem, Flags@\wgAdd{ = \{\}}@);
  template<class Flags@\wgAdd{ = element_aligned_tag}@>
    @\wgAdd{constexpr}@ void copy_to(value_type* mem, Flags@\wgAdd{ = \{\}}@)@\wgAdd{ const}@;

  // \ref{simd.mask.subscr}, \wgChange{S}{\tcode{simd_mask} s}ubscript operators
  @\wgAdd{constexpr }@reference operator[](size_t);
  @\wgAdd{constexpr }@value_type operator[](size_t) const;

  // \ref{simd.mask.unary}, \wgChange{U}{\tcode{simd_mask} u}nary operators
  @\wgAdd{constexpr }@simd_mask operator!() const noexcept;

  // \ref{simd.mask.binary}, \wgChange{B}{\tcode{simd_mask} b}inary operators
  friend @\wgAdd{constexpr }@simd_mask operator&&(const simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask operator||(const simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask operator&(const simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask operator|(const simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask operator^(const simd_mask&, const simd_mask&) noexcept;

  // \ref{simd.mask.cassign}, \wgChange{C}{\tcode{simd_mask} c}ompound assignment
  friend @\wgAdd{constexpr }@simd_mask& operator&=(simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask& operator|=(simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask& operator^=(simd_mask&, const simd_mask&) noexcept;

  // \ref{simd.mask.comparison}, \wgChange{C}{\tcode{simd_mask} c}omparisons
  friend @\wgAdd{constexpr }@simd_mask operator==(const simd_mask&, const simd_mask&) noexcept;
  friend @\wgAdd{constexpr }@simd_mask operator!=(const simd_mask&, const simd_mask&) noexcept;
};
\end{codeblock}
...
\pnum
Every specialization of \tcode{simd_mask} \wgChange{shall be}{is} a complete type. The specialization \tcode{simd_mask<T, Abi>} is supported if \tcode{T} is a vectorizable type and
\begin{itemize}
  \item \tcode{Abi} is \tcode{simd_abi::scalar}, or
  \item \tcode{Abi} is \tcode{simd_abi::fixed_size<N>}, with \tcode{N} constrained as defined in (\ref{simd.abi}).
\end{itemize}
...
\begin{codeblock}
@\wgAdd{constexpr }@explicit operator @\impdef@() const;
@\wgAdd{constexpr }@explicit simd_mask(const @\impdef@& init) const;
\end{codeblock}
...
\rSec2[simd.mask.ctor]{\texorpdfstring{\wgChange{C}{\tcode{simd_mask} c}onstructors}{simd_mask constructors}}

\begin{itemdecl}
@\wgAdd{constexpr }@explicit simd_mask(value_type x) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class U@\wgAdd{, class UAbi}@>
  @\wgAdd{constexpr explicit(sizeof(U) != sizeof(T)) }@simd_mask(const simd_mask<U, @\wgChange{simd_abi::fixed_size<size()>}{UAbi}@>& x) noexcept;
\end{itemdecl}
...
\begin{itemdescr}
  ...
  \pnum\remarks
  This constructor shall not participate in overload resolution unless \wgChange{\tcode{abi_type} is \tcode{simd_abi::fixed_size<size()>}}{\tcode{simd_size_v<U, UAbi> == size()}}.
\end{itemdescr}
...
\begin{itemdecl}
template<class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@simd_mask(const value_type* mem, Flags@\wgAdd{ = \{\}}@);
\end{itemdecl}
...
\rSec2[simd.mask.copy]{\texorpdfstring{\wgChange{C}{\tcode{simd_mask} c}opy}{simd_mask copy} functions}

\begin{itemdecl}
template<class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@void copy_from(const value_type* mem, Flags@\wgAdd{ = \{\}}@);
\end{itemdecl}
...
\begin{itemdecl}
template<class Flags@\wgAdd{ = element_aligned_tag}@> @\wgAdd{constexpr }@void copy_to(value_type* mem, Flags@\wgAdd{ = \{\}}@);
\end{itemdecl}
...
\rSec2[simd.mask.subscr]{\texorpdfstring{\wgChange{S}{\tcode{simd_mask} s}ubscript}{simd_mask subscript} operators}
...
\rSec2[simd.mask.unary]{\texorpdfstring{\wgChange{U}{\tcode{simd_mask} u}nary}{simd_mask unary} operators}
...
\rSec2[simd.mask.binary]{\texorpdfstring{\wgChange{B}{\tcode{simd_mask} b}inary}{simd_mask binary} operators}

\begin{itemdecl}
friend @\wgAdd{constexpr }@simd_mask operator&&(const simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask operator||(const simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask operator& (const simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask operator| (const simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask operator^ (const simd_mask& lhs, const simd_mask& rhs) noexcept;
...
\rSec2[simd.mask.cassign]{\texorpdfstring{\wgChange{C}{\tcode{simd_mask} c}ompound}{simd_mask compound} assignment}

\begin{itemdecl}
friend @\wgAdd{constexpr }@simd_mask& operator&=(simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask& operator|=(simd_mask& lhs, const simd_mask& rhs) noexcept;
friend @\wgAdd{constexpr }@simd_mask& operator^=(simd_mask& lhs, const simd_mask& rhs) noexcept;
\end{itemdecl}
...
\rSec2[simd.mask.comparison]{\texorpdfstring{\wgChange{C}{\tcode{simd_mask} c}omparisons}{simd_mask comparisons}}
...
\rSec2[simd.mask.reductions]{\texorpdfstring{\wgChange{R}{\tcode{simd_mask} r}eductions}{simd_mask reductions}}

\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@bool all_of(const simd_mask<T, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@bool any_of(const simd_mask<T, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@bool none_of(const simd_mask<T, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@bool some_of(const simd_mask<T, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@int popcount(const simd_mask<T, Abi>& k) noexcept;
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@int find_first_set(const simd_mask<T, Abi>& k);
\end{itemdecl}
...
\begin{itemdecl}
template<class T, class Abi> @\wgAdd{constexpr }@int find_last_set(const simd_mask<T, Abi>& k);
\end{itemdecl}
...
\rSec2[simd.mask.where]{\tcode{where} functions}
...
\begin{itemdescr}
  \pnum\returns
  An object (\ref{simd.whereexpr}) with \tcode{mask} and \tcode{data} initialized with \tcode{k} and \tcode{v} respectively.
\end{itemdescr}
