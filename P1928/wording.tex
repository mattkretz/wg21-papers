...
\begin{itemdescr}
  ...
  \begin{itemize}
    ...
    \item \tcode{common_type_t<\mathcommonsimd<T0>, T1>}, if \tcode{sizeof...(Ts)}
      equals $2$ and \tcode{\mathfloatingpoint<T0>} is \tcode{true}; otherwise

    \item \tcode{common_type_t<T0, \mathcommonsimd<T1>>}, if \tcode{sizeof...(Ts)}
      equals $2$ and \tcode{\mathfloatingpoint<T1>} is \tcode{true}; otherwise

    \item \tcode{common_type_t<\mathcommonsimd<T0, T1>, TRest...>}, if
      \tcode{common_type_t<\mathcommonsimd<T0>, \mathcommonsimd<T1>>} is valid and denotes a type;
      otherwise
...
namespace std {
  ...
  template<@\mathfloatingpoint@ V> constexpr @\deducedsimd@<V> modf(const
  type_identity_t<@\deducedsimd@<V>>& value, V* iptr);
...
\pnum
Unless otherwise specified below, each mathematical function declared in \tcode{<simd>} returns a
prvalue \tcode{result} of a data-parallel type, where
the value of each element approximates the value obtained by
...
\begin{itemdescr}
  ...
  except it is unspecified whether \tcode{errno} is accessed.
\end{itemdescr}
