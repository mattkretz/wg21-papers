...
\begin{itemdescr}
  ...
  \begin{itemize}
    ...
    \item if \code{ranges::size(r)} is a constant expression then \code{ranges::size(r)} $\ge$
      \tcode{V::size()}.
  \end{itemize}
  ...
  \pnum\effects
  Initializes the $i^\text{th}$ element with \tcode{mask[$i$] ? static_cast<T>(ranges::data(r)[$i$])
  : T()} \foralli[V::].
...
\begin{itemdescr}
  ...
  \pnum\effects
  \Foralli[basic_simd<T, Abi>::], if \tcode{mask[$i$]} is \code{true}, evaluates \code{ranges::data(r)[$i$] = v[$i$]}.
\end{itemdescr}
