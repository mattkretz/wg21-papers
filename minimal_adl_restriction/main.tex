...
\label{sec:convopadl}
Observe the inconsisteny in \lst{motivateADLconvop}.
...
\TODO{
\todoitem Release patched GCC 12.1 and link to it from the paper.
}

\subsection{Opt-out/in ADL}
\TODO{
\todoitem \lstinline[style=Vc]@struct foo : adl bar { /* ... */ };@ opts out of 
considering implicit associated entities; \code{bar} (and its associated entities) are the 
only associated entities
\todoitem \lstinline[style=Vc]@struct meow : adl void { /* ... */ };@ has no associated 
entities.
}
