...
\pnum
\constraints
\wgRem{\tcode{U} satisfies \tcode{\exposconcept{explicitly-convertible-to}<value_type>}.}
\begin{itemize}
 \wgItemAdd[%
   \tcode{U} satisfies \tcode{\libconcept{convertible_to}<value_type>} and
   \tcode{From} is not an arithmetic type and does not satisfy \exposconcept{constexpr-wrapper-like},]
 \wgItemAdd[%
