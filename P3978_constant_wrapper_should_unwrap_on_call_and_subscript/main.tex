...
\newcommand\wgDocumentNumber{P3978R0}
...
arguments}.
\begin{itemize}
  \item As a template argument passing a \code{1} the function sees an \code{int}.
  \item As a function argument passing a \code{cw<1>} the function does not see an \code{int} but a
...
stay in the type space, in which case it unwraps all operands/arguments and wraps the result.
