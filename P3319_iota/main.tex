...
\newcommand\wgDocumentNumber{P3319R3}
...
\newcommand\stdsimd{\std\code{simd\MayBreak::\MayBreak}}
...
Consider \code{simd_iota<simd<char, 512>>} where \code{is_signed_v<char>} is
\code{true}.
...
I was using \code{simd_iota} in test code and encountered both cases.
...
and making \code{simd_iota} ill-formed unnecessarily made my test cases harder
to write.

Granted, most people won't use \code{simd_iota} in order to compare it against
\std\code{iota}.
...
Therefore, I prefer making \code{simd_iota} ill-formed if the \simd width is
