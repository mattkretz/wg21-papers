...
\begin{codeblock}
@\wgRem{template<class V, class T> using \mbox{\exposid{make-compatible-simd-t} = \seebelow}; \mbox{\rmfamily\itshape// \expos}}@
...
\begin{itemdecl}
@\wgRem{template<class V, class T> using \mbox{\exposid{make-compatible-simd-t} = \seebelow};}@
\end{itemdecl}
...
\pnumRem
\wgRem{\mbox{\tcode{\exposid{make-compatible-simd-t}<V, T>}} is an alias for}
\begin{itemize}
 \wgItemRem[
   \mbox{\tcode{\exposid{deduced-vec-t}<T>}}, if that type is not \tcode{void},
 otherwise]
 \wgItemRem[
...
\mbox{\exposconcept{explicitly-convertible-to}}.}
}
...
\begin{codeblock}
    ...
    template<@\wgChange{class}{\mbox{\exposconcept{explicitly-convertible-to}<value_type>}}@ U>
      constexpr explicit(@\seebelow@) basic_vec(U&& value) noexcept;
    @\wgAdd{template<\mbox{\exposconcept{simd-consteval-broadcast-arg}<value_type>} U>}@
...
\begin{itemdecl}
@\wgAdd{template<\mbox{\exposconcept{simd-consteval-broadcast-arg}<value_type>} U> consteval basic_vec(U\&\& x)}@
\end{itemdecl}
