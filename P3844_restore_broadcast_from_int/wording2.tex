...
@\wgRem{template<class... Ts>}@
  @\wgRem{concept \defexposconceptnc{math-floating-point} =}@                                      @\wgRem{\rmfamily\itshape// \expos}@
    @\wgRem{(\exposconceptnc{simd-floating-point}<\exposidnc{deduced-vec-t}<Ts>> || ...);}@
...
\begin{itemdecl}
template<class BinaryOperation, class T>
  concept @\defexposconceptnc{reduction-binary-operation}@ =
\end{itemdecl}
\end{wgText}
