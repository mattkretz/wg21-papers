...
@\wgAdd{template<class From, class To>}@
  @\wgAdd{\mbox{concept \defexposconceptnc{simd-broadcast-arg} = \libconcept{constructible_from}}<To, From>;}@         @\wgAdd{\mbox{\rmfamily\itshape// \expos}}@
...
\pnumAdd
\wgAdd{%
Types \tcode{From} and \tcode{To} satisfy
\mbox{\exposconcept{simd-consteval-broadcast-arg}} only if
\mbox{\tcode{\libconcept{convertible_to}<From, To>}} is satisfied,
\tcode{remove_cvref_t<From>} is an arithmetic type, and the conversion from
\tcode{remove_cvref_t<From>} to \tcode{To} is not value-preserving.
}
\end{itemdescr}
\end{wgText}
